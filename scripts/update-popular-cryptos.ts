...
function isEligible(
    symbol: string,
    listedSymbols: ReadonlySet<string>
): boolean {
...
function commitFileAtomically(path: string, fileContent: string): void {
    ...
    try {
        ...
        try {
            ...
            console.warn(
                `[update-popular-cryptos] temp file cleanup failed: ${tempPath}`,
                cleanupError
            );
        }
    }
...
function printResults(ranked: CryptoRankEntry[]): void {
    ...
    ranked.forEach((e, i) => {
        ...
        const cap = formatMarketCap(e.marketCap).padStart(
            TABLE_MARKET_CAP_COL_WIDTH
        );
