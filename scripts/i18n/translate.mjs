 ...
 *
 * **손으로 번역한 키는 해시도 같이 갱신한다.** 해시가 없거나 옛 ko 값의 해시면
 * 이 스크립트는 그 키를 "변경분"으로 보고 다시 번역한다 — 2026-09-19에는 해시가
 * 빠진 1,611키 때문에 새 키 140개를 번역하려던 실행이 승인된 en 1,633키를 전부
 * 다시 번역하기 시작했다. 실행 전에 `--dry-run`으로 "번역 필요" 수를 먼저 본다.
 *
