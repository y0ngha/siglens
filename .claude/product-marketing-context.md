...
**Version:** 0.9.x (post PR #434 — options 분석 surface)
**Website:** https://siglens.io
**Tagline:** AI가 분석하는 미국 주식 — 기술적 분석, 펀더멘털, 뉴스, 공포 탐욕 지수, 옵션 시장, 종합 결론까지 한 번에

Siglens는 미국 주식 분석의 전 워크플로우를 AI로 자동화합니다. 사용자는 티커를 입력하기만 하면 ① 차트 기반 기술적 분석, ② PER, ROE, 재무건전성 등 펀더멘털 분석, ③ 뉴스 호재/악재 분위기, 어닝 및 실적 캘린더, 애널리스트 등급 변경 분석, ④ 종목별 단기 매수 분위기를 0~100 점수로 보여주는 공포 탐욕 지수, ⑤ 만기별 Max Pain · Put/Call · ATM IV · Implied Move · Open Interest 분포까지 한국어로 해석하는 옵션 시장 분석, ⑥ 이 다섯 축을 통합한 AI 종합 결론과 시나리오 분석까지 한 화면에서 받습니다.
...
### 분석 페이지 (6개)
- **기술적 분석 (`/[symbol]`)** — 25종 보조지표 + 캔들 패턴 45종 + 차트 패턴 + AI 종합 리포트 + 보조 뉴스 카드 (NewsAugment) + 옵션 시그널 카드 3종(ATM IV·Put/Call·Max Pain, 옵션 시장이 있는 종목만)
...
- **옵션 분석 (`/[symbol]/options`)** — AI가 옵션 시장을 한국어로 해석. 4개 메트릭 카드(Max Pain, Put/Call Ratio, ATM IV, Implied Move) + Strike별 Open Interest 분포 차트(Call/Put bar, Max Pain dashed, 현재가 solid) + 만기별 옵션 체인 테이블 + 만기 선택 탭(WAI tabs 키보드 네비). 옵션 시장이 없는 종목은 `hasOptionsMarket` 가드로 빈 상태 안내 + `robots: noindex`. 시장 시간대(ET)에 따라 cacheLife 프로파일(open/closed/weekend) 자동 전환
- **AI 종합 분석 (`/[symbol]/overall`)** — 명시 트리거(CTA 버튼) + 4축(기술·펀더·뉴스·옵션) 의존성 진행 UI + fear-greed 지수까지 inject한 **5축 통합 결론**, 시나리오 분석, 위험 요인
...
- **7개 카테고리 / 67 Skills** — `indicators/`(25), `patterns/`(차트 패턴), `strategies/`(엘리어트·다이버전스 등), `support-resistance/`(피보나치·피벗), `candlesticks/`(도지·해머 등), `fundamental/`(value/growth/quality investing), `news/`(event-driven/macro-impact/earnings-reaction)

### 인증 & 회원
- **회원가입은 옵션 (`/signup`, `/login`)** — 비회원도 모든 기본 분석 기능 사용 가능. 가입 시 tier 기반 추가 혜택
- **3단계 Tier** — `free` (비회원/기본 회원) / `member` / `pro`. tier가 올라갈수록 Pro AI 모델 선택지 확대 + BYOK + 분석 한도 상향
- **인증 방식** — 이메일/비밀번호(bcrypt 해싱) + 소셜 로그인 (Google 활성, Kakao 구성)
- **이메일 인증 & 비밀번호 재설정** — `/forgot-password`, `/reset-password` + 메일 발송(Resend)
...
### 인프라 & 자동화
- **Drizzle ORM + Postgres** — `news`, `earnings_reports` 테이블 (어닝 데이터는 종목별 on-demand FMP fetch + DB upsert)
- **FMP `/stable` API** — 뉴스, 어닝, 재무제표, 애널리스트 등급, 섹터 데이터
- **Alpaca Markets** — 시장 데이터(15분 지연 IEX)
- **Yahoo Finance (yahoo-finance2)** — 옵션 시장 스냅샷(체인·만기·OI·IV·Greeks). `OptionsDataProvider` 인터페이스(siglens-core) 뒤에 `YahooOptionsAdapter`로 어댑터화 — 향후 Tradier fallback 추가 가능
...
- Lightweight Charts 5.1 (가격 차트) + 자체 SVG (Open Interest 차트)
...
- @y0ngha/siglens-core (외부화된 도메인 분석 로직 — fear-greed 5-factor, options domain types + `OptionsDataProvider`/`computeOptionsSignals`/`mapExpirationsToSlots`)
...
/[symbol]/options            → 옵션 분석 (priority 0.78, hourly, WebPage + Breadcrumb + FAQPage). 옵션 시장이 없는 종목은 sitemap에서 제외 + page-level `robots: noindex`
...
- 동적 metadata via `generateMetadata()` — symbol 5종(기술/뉴스/펀더/공포탐욕/옵션 + 종합), market, backtesting
...
- Sitemap: 홈(1.0) + market과 backtesting(0.9) + 200+ 티커 × 최대 6개 페이지(0.75~0.85) + privacy/terms(0.3). 옵션 페이지는 `hasOptionsMarket(ticker)` 통과한 종목만 emit
- Robots: 인덱싱 허용, `/api/` disallow, 인증/계정 페이지는 noindex, 옵션 시장 없는 종목의 옵션 페이지는 page-level noindex
...
- **Korean-first** — UI·분석 리포트·메타·뉴스 sentiment·옵션 해석 모두 한국어
...
| 5축 분석 / 기술 펀더 뉴스 공포탐욕 옵션 통합 | `/[symbol]/overall` |

## SEO Keywords (Korean) — Options (NEW)

| Keyword | Mapped Page |
|---------|-------------|
| {티커} 옵션 | `/[symbol]/options` |
| {티커} 옵션 분석 | `/[symbol]/options` |
| {티커} Max Pain | `/[symbol]/options` |
| {티커} Put Call Ratio | `/[symbol]/options` |
| {티커} Open Interest | `/[symbol]/options` |
| {티커} Implied Volatility | `/[symbol]/options` |
| {티커} 내재 변동성 | `/[symbol]/options` |
| {티커} 옵션 시장 | `/[symbol]/options` |
| {티커} 옵션 만기 | `/[symbol]/options` |
| {티커} 콜 풋 비율 | `/[symbol]/options` |
| 옵션 분석 | `/[symbol]/options` |
| 옵션 시장 | `/[symbol]/options` |
| Max Pain 종목 | `/[symbol]/options` |
| Put/Call Ratio | `/[symbol]/options` |
| Implied Volatility | `/[symbol]/options` |
| Open Interest | `/[symbol]/options` |
| ATM IV | `/[symbol]/options` |
| Implied Move | `/[symbol]/options` |
| 미국 주식 옵션 한국어 | `/[symbol]/options` |
...
| {종목명} 옵션/Max Pain | TSLA 옵션, NVDA Max Pain, 애플 Put Call Ratio |
...
PR #434 (2026-05-21) 머지 — 6개 분석 페이지(기술/펀더/뉴스/공포탐욕/**옵션**/종합) + 백테스팅 페이지 + 회원 시스템 + BYOK + 3개 AI 프로바이더 모델 선택이 모두 라이브. PR #434에서 **옵션 분석 surface**가 신규 추가됨: `/[symbol]/options`(AI 옵션 분석 + Max Pain/P/C/ATM IV/Implied Move 메트릭 + Strike별 OI 차트 + 옵션 체인) 및 차트 페이지 하단의 3개 옵션 시그널 카드(ATM IV / Put/Call / Max Pain). 데이터 소스는 yahoo-finance2(Phase 1, snapshot-only), `OptionsDataProvider` 인터페이스로 어댑터화되어 Tradier fallback 추가 여지 보존. **종합 분석은 5축 통합(기술+펀더+뉴스+공포탐욕+옵션)** 으로 라이브. 11개 섹터 신호 스캐너(`/market`), AI 챗봇(컨텍스트 자동 전환), 67 Skills(7 카테고리), 200+ 티커 × 최대 6 페이지 = 1200+ 분석 페이지가 sitemap에 등록(옵션 페이지는 `hasOptionsMarket` 통과 종목만).
