...
**Version:** 0.8.x (post PR #413 — siglens-core 0.7.3)
**Website:** https://siglens.io
**Tagline:** AI가 분석하는 미국 주식 — 기술적 분석, 펀더멘털, 뉴스, 종합 결론까지 한 번에

Siglens는 미국 주식 분석의 전 워크플로우를 AI로 자동화합니다. 사용자는 티커를 입력하기만 하면 ① 차트 기반 기술적 분석, ② PER·ROE·재무건전성 등 펀더멘털 분석, ③ 뉴스 sentiment·어닝 캘린더·애널리스트 등급 변경 분석, ④ 이 셋을 통합한 AI 종합 결론과 시나리오 분석까지 한 화면에서 받습니다.
...
- **AI 4축 분석 자동화** — 티커만 입력하면 기술적·펀더멘털·뉴스·종합 4개 분석을 모두 받습니다
- **통합 결론(/[symbol]/overall)** — 3축(기술/펀더/뉴스) 결과를 묶어 시나리오 분석과 위험 요인을 도출
...
- **뉴스 분석 (`/[symbol]/news`)** — 최신 뉴스 리스트(sentiment 뱃지 + 한국어 요약), 종합 AI 뉴스 분석, 이벤트 캘린더(다음 어닝·최근 보고서), 애널리스트 액션. 페이지 진입 시 FMP에서 뉴스 fetch + Gemini Flash-lite로 카드별 자동 분석
- **펀더멘털 분석 (`/[symbol]/fundamental`)** — 회사 프로필, 밸류에이션(PER·PSR·EPS), 동종업계, 수익성(ROE·마진), 성장성, 재무건전성, 미래 방향(애널리스트 컨센서스·목표가), 섹터 방향 + AI 종합 펀더 분석
- **AI 종합 분석 (`/[symbol]/overall`)** — 명시 트리거(CTA 버튼) + 3축(기술/펀더/뉴스) 의존성 진행 UI + 한 줄 정의·3축 요약·통합 결론·시나리오 분석·위험 요인
...
- @y0ngha/siglens-core 0.7.3 (외부화된 도메인 분석 로직)
...
/[symbol]                    → 기술적 분석 (priority 0.8, daily, WebPage + Breadcrumb)
/[symbol]/news               → 뉴스 분석 (priority 0.78, hourly, Breadcrumb)
/[symbol]/fundamental        → 펀더멘털 분석 (priority 0.75, weekly, Breadcrumb)
/[symbol]/overall            → AI 종합 분석 (priority 0.73, weekly, Breadcrumb)
...
- Sitemap: 홈(1.0) + market·backtesting(0.9) + 200+ 티커 × 4축(0.73~0.8) + privacy/terms(0.3)
...
| 3축 분석 / 기술 펀더 뉴스 통합 | `/[symbol]/overall` |
...
| {종목명} 뉴스 | 마이크로소프트 뉴스, 테슬라 뉴스 sentiment |
| {종목명} 어닝 | 애플 어닝 발표, 엔비디아 어닝 일정 |
...
PR #413 (2026-05) 머지 직후 — 4축 분석(기술/펀더/뉴스/종합) + 백테스팅 페이지 + 회원 시스템 + BYOK가 모두 라이브. 11개 섹터 신호 스캐너(`/market`), AI 챗봇(컨텍스트 자동 전환), 67+ Skills, 200+ 티커 × 4축 = 800+ 분석 페이지가 sitemap에 등록.
