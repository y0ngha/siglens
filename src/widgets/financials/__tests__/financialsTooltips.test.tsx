...
describe('financialsTooltips', () => {
    it('FcfTooltip renders FCF explanation text', () => {
        ...
        expect(
            screen.getByText(/설비투자\(CapEx\)를 빼고/)
        ).toBeInTheDocument();
        expect(
            screen.getByText(/배당·자사주·빚 상환의 재원/)
        ).toBeInTheDocument();
    });
    ...
    it('CapExTooltip renders capital expenditure explanation text', () => {
        ...
        expect(screen.getByText(/성장 투자로 봐요/)).toBeInTheDocument();
    });
    ...
    it('FcfMarginTooltip renders FCF margin explanation text', () => {
        ...
        expect(
            screen.getByText(/매출을 현금으로 잘 바꾼다/)
        ).toBeInTheDocument();
    });
    ...
    it('GrossMarginTooltip renders gross margin explanation text', () => {
        ...
        expect(screen.getByText(/기본 수익성이 좋다/)).toBeInTheDocument();
    });
});
