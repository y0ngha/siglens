...
import { HURST_RANDOM_WALK_LEVEL } from '../constants/indicatorLevels';
import { buildSeriesDataFromValues } from '../utils/seriesDataUtils';

interface UseHurstChartParams {
...
export function useHurstChart({
    ...
    useEffect(() => {
        ...
        if (!seriesRef.current) {
            ...
            seriesRef.current.createPriceLine({
                price: HURST_RANDOM_WALK_LEVEL,
                color: CHART_COLORS.hurstReference,
                lineWidth,
    ...
    useEffect(() => {
        ...
        const { hurst } = indicators;
        if (!hurst.length) return;

        if (!seriesRef.current) return;

        seriesRef.current.setData(buildSeriesDataFromValues(bars, hurst));
