...
interface UseWilliamsRChartParams {
...
export function useWilliamsRChart({
    chartRef,
    bars,
    indicators,
    ...
    useEffect(() => {
        ...
        if (!seriesRef.current) {
            ...
            seriesRef.current.createPriceLine({
                ...
                color: CHART_COLORS.williamsROverbought,
                lineWidth,
            ...
            seriesRef.current.createPriceLine({
                price: WILLIAMS_R_OVERSOLD_LEVEL,
                color: CHART_COLORS.williamsROversold,
                lineWidth,
                lineStyle: LineStyle.Dashed,
                axisLabelVisible: false,
                title: '',
    ...
    useEffect(() => {
        ...
        if (!williamsR.length) return;

        if (!seriesRef.current) return;

        seriesRef.current.setData(buildSeriesDataFromValues(bars, williamsR));
