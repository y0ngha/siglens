...
import { CMF_ZERO_LEVEL } from '../constants/indicatorLevels';
import { buildSeriesDataFromValues } from '../utils/seriesDataUtils';

interface UseCmfChartParams {
...
export function useCmfChart({
    ...
    useEffect(() => {
        ...
        if (!seriesRef.current) {
            ...
            seriesRef.current.createPriceLine({
                price: CMF_ZERO_LEVEL,
                color: CHART_COLORS.cmfZero,
                lineWidth,
    ...
    useEffect(() => {
        ...
        const { cmf } = indicators;
        if (!cmf.length) return;

        if (!seriesRef.current) return;

        seriesRef.current.setData(buildSeriesDataFromValues(bars, cmf));
