...
function makeChartRef(chart: unknown = null) {
    return {
        current: chart,
    } as Parameters<typeof useConnorsRsiChart>[0]['chartRef'];
}
...
const EMPTY_INDICATORS = { connorsRsi: [] } as unknown as IndicatorResult;

const FILLED_INDICATORS = {
    connorsRsi: [null, null, 50, 92, 8],
...
describe('useConnorsRsiChart', () => {
    ...
    it('creates Connors RSI series when visible with chart', () => {
        renderHook(() =>
            useConnorsRsiChart({
                chartRef: makeChartRef(makeChart()),
                bars: FAKE_BARS,
