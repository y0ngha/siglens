...
function makeChartRef(chart: unknown = null) {
    return {
        current: chart,
    } as Parameters<typeof useBollingerPercentBChart>[0]['chartRef'];
}
...
// bollingerDerived is an object array ({ pctB, bandwidth }), unlike the
// number[] of other oscillators — the hook extracts pctB before setData.
const EMPTY_INDICATORS = { bollingerDerived: [] } as unknown as IndicatorResult;

const FILLED_INDICATORS = {
    bollingerDerived: [
        { pctB: null, bandwidth: null },
        { pctB: 0.1, bandwidth: 0.5 },
        { pctB: 0.9, bandwidth: 0.8 },
    ],
...
describe('useBollingerPercentBChart', () => {
    beforeEach(() => {
        vi.clearAllMocks();
    });
    ...
    it('creates Bollinger %B series when visible with chart', () => {
        renderHook(() =>
            useBollingerPercentBChart({
                chartRef: makeChartRef(makeChart()),
                bars: FAKE_BARS,
