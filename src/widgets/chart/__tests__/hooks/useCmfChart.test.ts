...
function makeChartRef(chart: unknown = null) {
    return { current: chart } as Parameters<typeof useCmfChart>[0]['chartRef'];
}
...
const EMPTY_INDICATORS = { cmf: [] } as unknown as IndicatorResult;

const FILLED_INDICATORS = {
    cmf: [null, null, 0.1, -0.2, 0.3],
...
describe('useCmfChart', () => {
    ...
    it('creates CMF series when visible with chart', () => {
        renderHook(() =>
            useCmfChart({
                chartRef: makeChartRef(makeChart()),
                bars: FAKE_BARS,
