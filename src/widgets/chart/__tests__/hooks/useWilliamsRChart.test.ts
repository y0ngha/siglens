...
function makeChartRef(chart: unknown = null) {
    return {
        current: chart,
    } as Parameters<typeof useWilliamsRChart>[0]['chartRef'];
}
...
const EMPTY_INDICATORS = { williamsR: [] } as unknown as IndicatorResult;

const FILLED_INDICATORS = {
    williamsR: [null, null, -50, -15, -85],
...
describe('useWilliamsRChart', () => {
    ...
    it('creates Williams %R series when visible with chart', () => {
        renderHook(() =>
            useWilliamsRChart({
                chartRef: makeChartRef(makeChart()),
                bars: FAKE_BARS,
