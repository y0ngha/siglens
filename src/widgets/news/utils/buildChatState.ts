...
import type { NewsAnalysisState } from '../hooks/useNewsAnalysis';
...
// timeframe은 뉴스 분석에서 의미가 없어 null — FloatingChatButton 측에서
// DEFAULT_TIMEFRAME으로 fallback 한다.
export function buildChatState(state: NewsAnalysisState): SymbolChatState {
    if (state.status === 'done') {
        return {
            context: { kind: 'news', payload: state.result },
            timeframe: null,
