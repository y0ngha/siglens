...
export function Header({
    ...
    // On ai.siglens.io the header is the same component with a link base set;
    // there the lockup reads as one brand ("SIGLENS AI") and the logo goes to
    // the AI home (a new conversation), not back to the main site.
    ...
    return (
        <header className="sticky top-0 z-50 border-b border-secondary-700 bg-secondary-900/90 backdrop-blur-md supports-backdrop-filter:bg-secondary-900/75">
            ...
            <div className="flex h-14 items-center gap-2 px-4 sm:gap-4">
                {/* Logo lockup: `[icon] SIGLENS` + `AI`. Two anchors because they
                    are two destinations on the main host (site home / AI product);
                    visually one wordmark, so the gap is tighter than the row's. */}
                <div className="flex shrink-0 items-center gap-1">
                    ...
                    */}
                        ...
                        <span
                            translate="no"
                            // On the ai host the wordmark is the product's brand, so it
                            // stays visible on phones too; the main host keeps the icon-only
                            // mobile logo to leave room for search/theme/account.
                            className={cn(
