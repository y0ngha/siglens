 ...
 *
 * 그래서 스캐너를 **자기 규칙으로 검사하지 않는다.** 파서가 내놓은 주석 목록을
 * 정답지로 두고, 스캐너가 비운 좌표와 대조한다.
 *
 * **보증 범위를 정확히 적어 둔다**: 정답지도 스캐너와 같은 `@babel/parser`를
 * 쓴다. 따라서 이 테스트가 잡는 것은 "스캐너의 손수 만든 문자열 순회가 파서의
 * 주석 판정에서 이탈하는 것"이고, babel 자신이 오파싱하는 경우는 원리적으로 못
 * 잡는다. 한때 이 주석이 "독립적으로 구현된 파서"라고 적혀 있었는데 사실이
 * 아니었다 — 문서가 실제보다 강한 보증을 약속하면 다음 사람이 그만큼 덜 본다.
 * 진짜 독립성이 필요해지면 정답지를 다른 파서로 바꿔야 한다.
 */
