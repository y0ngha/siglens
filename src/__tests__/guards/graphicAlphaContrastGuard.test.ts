 ...
 * → 2.27). 별표와 슬래시를 붙여 `bg-ui-⟨토큰⟩/60`처럼 줄여 쓰면 이 주석이
 * 거기서 끝나 파일이 통째로 파싱 불가가 된다 — 실제로 한 번 그렇게 깨졌다.
 * 매번 인스턴스만 고쳤고, 그때마다 "이 파일은 봤으니 됐다"는 신호가 남았다.
...
/**
 * 정보를 나르지 않는 그래픽. **왜 대상이 아닌지 함께 적는다** —
 * 근거 없는 예외는 다음 사람에게 "여기는 규칙이 없다"로 읽힌다.
 */
/*
 * 지금은 비어 있다. 진행 막대의 트랙, 모바일 메뉴 스크림, 스켈레톤
 * 자리표시자는 예외로 적을 필요가 없었다 — 전부 중립 램프거나 `black`이라
 * 위 계열 필터에서 이미 빠진다. 예외를 적어두면 스캐너가 닿지도 않는 항목이
 * 규칙처럼 읽히므로, 실제로 걸리는 것만 여기 넣는다.
 */
