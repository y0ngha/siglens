...
const makePatternSummary = (
    overrides?: Partial<AnalysisResponse['patternSummaries'][number]>
): AnalysisResponse['patternSummaries'][number] => ({
    patternName: '테스트 패턴',
    skillName: '테스트 스킬',
    detected: true,
    trend: 'bullish',
...
const makeSkillResult = (
    overrides?: Partial<AnalysisResponse['skillResults'][number]>
): AnalysisResponse['skillResults'][number] => ({
    skillName: '테스트 스킬',
    trend: 'bullish',
    summary: '테스트 요약',
    confidenceWeight: 0,
    ...
    describe('enrichAnalysisWithConfidence', () => {
        ...
        describe('불변성', () => {
            it('원본 analysis 객체를 변경하지 않는다', () => {
                ...
                expect(analysis.patternSummaries[0].confidenceWeight).toBe(0);
            });
