...
describe('resolveDefaultModelForProvider', () => {
    ...
    describe('when the top priority model is blocked', () => {
        it('returns the second priority claude model when the first is blocked', () => {
            ...
            const result = resolveDefaultModelForProvider('claude', allowedModels);
        ...
        it('returns the second priority gemini model when the first is blocked', () => {
            ...
            const result = resolveDefaultModelForProvider('gemini', allowedModels);
        ...
        it('returns the second priority chatgpt model when the first is blocked', () => {
            ...
            const result = resolveDefaultModelForProvider('chatgpt', allowedModels);
    ...
    describe('when only the lowest priority model is allowed', () => {
        it('returns the bottom priority claude model', () => {
            const lowestPriority = CLAUDE_MODEL_PRIORITY[CLAUDE_MODEL_PRIORITY.length - 1];
            const result = resolveDefaultModelForProvider('claude', [lowestPriority]);
            expect(result).toBe(lowestPriority);
        });

        it('returns the bottom priority gemini model', () => {
            const lowestPriority = GEMINI_MODEL_PRIORITY[GEMINI_MODEL_PRIORITY.length - 1];
            const result = resolveDefaultModelForProvider('gemini', [lowestPriority]);
            expect(result).toBe(lowestPriority);
        });

        it('returns the bottom priority chatgpt model', () => {
            const lowestPriority = CHATGPT_MODEL_PRIORITY[CHATGPT_MODEL_PRIORITY.length - 1];
            const result = resolveDefaultModelForProvider('chatgpt', [lowestPriority]);
            expect(result).toBe(lowestPriority);
        });
    });
    ...
    describe('when only models from a different provider are in allowedModels', () => {
        it('returns null for claude when only gemini models are allowed', () => {
            const result = resolveDefaultModelForProvider('claude', [...GEMINI_MODEL_PRIORITY]);
            expect(result).toBeNull();
        });

        it('returns null for gemini when only chatgpt models are allowed', () => {
            const result = resolveDefaultModelForProvider('gemini', [...CHATGPT_MODEL_PRIORITY]);
            expect(result).toBeNull();
        });

        it('returns null for chatgpt when only claude models are allowed', () => {
            const result = resolveDefaultModelForProvider('chatgpt', [...CLAUDE_MODEL_PRIORITY]);
            expect(result).toBeNull();
        });
    });
});
