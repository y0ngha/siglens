...
describe('calculateMA', () => {
  ...
  describe('입력 배열 길이가 period 이상일 때', () => {
    ...
    it('period번째 이후 값은 null이 아닌 숫자다', () => {
      const bars = makeBars(Array.from({ length: 30 }, (_, i) => 100 + i));
      const result = calculateMA(bars, 20);
      expect(result.slice(19).every((v) => typeof v === 'number')).toBe(true);
    });
    ...
    it('기간 5, 10, 20, 60, 120, 200을 지원한다', () => {
      const bars = makeBars(Array.from({ length: 200 }, (_, i) => 100 + i));
      [5, 10, 20, 60, 120, 200].forEach((period) => {
        const result = calculateMA(bars, period);
        expect(result).toHaveLength(200);
        expect(result.slice(0, period - 1).every((v) => v === null)).toBe(true);
        expect(result.slice(period - 1).every((v) => typeof v === 'number')).toBe(true);
      });
    });
  });
});