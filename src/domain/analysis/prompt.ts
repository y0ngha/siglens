...
const ANALYSIS_GUIDELINES = [
    ...
    '- entryPrices: Extract the numeric long entry price(s) from your entry analysis. If a range, provide [low, high]. If a single price, provide [price]. Must match the prices mentioned in the entry text field exactly.',
    '- stopLoss: Extract the stop-loss price for the long position from your exit analysis as a single number. Must match the stop-loss price mentioned in the exit text field.',
    '- takeProfitPrices: Extract the take-profit target price(s) for the long position from your exit analysis. Sort ascending (lowest price first). Must match the target prices mentioned in the exit text field.',
