...
const ANALYSIS_GUIDELINES = [
    ...
    '- Premium/Discount Zones: Prefer long (buy) entries in the discount zone. Avoid opening new long positions in the premium zone; wait for price to return to discount or equilibrium first. Equilibrium zone is neutral — wait for directional confirmation before entering.',
    ...
    '- Combine with trend: In a confirmed uptrend, bullish squeeze release is a high-confidence long (buy) entry. In a downtrend, wait for trend reversal confirmation before considering a long entry.',
    '',
    '### Conflicting Signals',
    ...
    '- LONG POSITION ONLY: actionRecommendation covers long (buy) positions exclusively. Do not generate short (sell) strategies, short entry points, short stop-loss levels, or short take-profit targets. Every field in actionRecommendation must reflect a long position perspective.',
    ...
    '- riskReward: Calculate the risk-reward ratio for the long position based on long entry price, stop-loss (below entry), and take-profit target(s). Express as a ratio (e.g., "stop-loss 3% vs target 9% → risk:reward = 1:3"). Do not calculate or mention short position risk-reward.',
    '- Write in accessible language that non-technical users can understand.',
    '- entryPrices: Extract the numeric long entry price(s) from your entry analysis. If a range, provide [low, high]. If a single price, provide [price]. Must match the prices mentioned in the entry text field exactly. Populate this field even when entryRecommendation is "wait" — use the conditional entry price(s) mentioned in the entry text.',
    '- stopLoss: Extract the stop-loss price for the long position from your exit analysis as a single number. Must match the stop-loss price mentioned in the exit text field. Populate this field even when entryRecommendation is "wait".',
    '- takeProfitPrices: Extract the take-profit target price(s) for the long position from your exit analysis. Sort ascending (lowest price first). Must match the target prices mentioned in the exit text field. Populate this field even when entryRecommendation is "wait".',
    '- entryRecommendation: Assess whether entering a long position right now is advisable. Use "enter" when conditions clearly favor immediate long entry (e.g., price near support with bullish confluence), "wait" when the setup is forming but not yet confirmed (e.g., approaching a key level, awaiting breakout), or "avoid" when risk outweighs reward (e.g., price extended from support, strong resistance overhead, high risk level). Only when entryRecommendation is "avoid" may entryPrices, stopLoss, and takeProfitPrices be left empty.',
