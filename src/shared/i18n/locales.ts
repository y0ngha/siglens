 ...
 * 언어 전환 UI를 화면에 낼지.
 *
 * **켠다(2026-09-15 사용자 결정).** 한국어 고정 공개 동안 감춰 뒀던 전환 수단을
 * 연다. 다국어 자체(라우팅·카탈로그·AI 산출물 언어)는 그동안에도 살아 있었고,
 * 바뀌는 건 사용자가 언어를 **고를 수단**이 헤더와 모바일 드로어에 생긴다는
 * 것뿐이다. 색인 게이트(`STATIC_INDEXABLE_LOCALES`)·hreflang·사이트맵은 이
 * 플래그와 무관하게 그대로다.
 *
 * 다시 감춰야 하면 이 값만 `false`로 바꾸면 된다 — 헤더와 모바일 드로어가 같은
