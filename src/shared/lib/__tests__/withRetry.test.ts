...
describe('withRetry 함수는', () => {
    ...
    describe('지수 백오프 스케줄에서', () => {
        it('200 → 400 → 800ms 순서로 sleep 을 호출한다', async () => {
            const fn = vi
                .fn()
    ...
    describe('backoffBudgetMs 옵션에서', () => {
        it('초과 시 다음 sleep 호출 없이 마지막 에러를 던진다', async () => {
        ...
        it('충분하면 정상 backoff 후 성공한다', async () => {
    ...
    describe('getRetryDelayMs 옵션에서', () => {
        it('non-null을 반환하면 지수 백오프 대신 해당 값으로 sleep을 호출한다', async () => {
            ...
            expect(getRetryDelayMs).toHaveBeenCalledWith(expect.any(Error), 0);
        });
        ...
        it('옵션이 없으면 기존 동작이 그대로 유지된다', async () => {
            const fn = vi
                .fn()
                .mockRejectedValueOnce(new Error('a'))
                .mockRejectedValueOnce(new Error('b'))
        ...
        it('반환한 값도 backoffBudgetMs 체크를 통과해야 sleep한다', async () => {
