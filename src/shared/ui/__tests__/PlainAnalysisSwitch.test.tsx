 ...
 * (`data-snapshot-prose`)에는 전문 용어가 그대로 들어 있어, 함께 접지 않으면
 * 쉽게보기를 켠 사람이 절반만 쉬운 화면을 보게 된다(실증으로 확인한 결함).
 *
 * 접기는 `SnapshotProseDisclosure`(`<details>`)가 하고, 이 컴포넌트는 루트 속성만
 * 세운다 — 그 섹션은 SEO 자산이라 DOM에서 지워서도, `display:none`으로 감춰서도
 * 안 된다(구글은 후자를 감가하고 접힌 아코디언은 동등하게 취급한다).
 */
describe('스냅샷 산문 연동', () => {
    afterEach(() => {
     ...
     * 표식을 세우면 스냅샷까지 접혀 **첫 화면에서 내용이 통째로 줄어든다.**
     * 봇에게 SEO 자산이 접힌 채 보이는 경로도 이 조건이 막는다.
     */
