...
export async function generateMetadata({
    ...
    const COPY = fearGreedCopyFor('crypto', t);
    ...
    // `getMarketFearGreedCryptoStatic`은 React.cache + unstable_cache 이중 래핑이라
    // 본문에서 다시 호출해도 fetch는 한 번만 실행된다.
    const view = await getMarketFearGreedCryptoStatic().catch((e: unknown) => {
        // 접두사는 CloudWatch 메트릭 필터(`siglens-market-data-loader-failed`)가 본다.
        console.error(
            '[FearGreedCryptoRoute] getMarketFearGreedCryptoStatic failed (metadata):',
            e
        );
        return null;
    });
...
export default async function FearGreedCryptoRoutePage({
    ...
    // 미국·한국 라우트와 동일 규약: 빈 ISR 캐시 동결을 막기 위해 throw 대신 빈 스냅샷으로
    // 폴백한다. `notFound()`는 절대 쓰지 않는다 — Suspense 안 notFound가 soft-404를
    // 만든 이력이 있다.
    const view: MarketFearGreedCryptoView =
        await getMarketFearGreedCryptoStatic().catch((e: unknown) => {
            console.error(
