...
import OgImage, {
    ...
    contentType,
    alt,
} from '@/app/[locale]/[symbol]/options/opengraph-image';
...
describe('[symbol]/options OG images', () => {
    describe('opengraph-image', () => {
        ...
        it('exports alt text containing options', () => {
            ...
            expect(alt).toContain('options');
        });

        it('calls buildSymbolOgImage with ticker and options label', async () => {
            await OgImage({
                params: Promise.resolve({ locale: 'ko', symbol: 'msft' }),
            });
    ...
    describe('twitter-image (re-export)', () => {
        it('re-exports same exports from opengraph-image', async () => {
            const twitter =
                await import('@/app/[locale]/[symbol]/options/twitter-image');
