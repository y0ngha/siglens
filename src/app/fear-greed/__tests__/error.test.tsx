...
import FearGreedError from '../error';

describe('FearGreedError (/fear-greed route error boundary)', () => {
    ...
    it('renders exactly one h1 (single-h1 contract is preserved even on error)', () => {
        const { container } = render(
            <FearGreedError error={error} reset={vi.fn()} />
        );
    ...
    it('wires the retry button to reset()', () => {
        const reset = vi.fn();
        render(<FearGreedError error={error} reset={reset} />);
    ...
    it('offers a working home link', () => {
        render(<FearGreedError error={error} reset={vi.fn()} />);
