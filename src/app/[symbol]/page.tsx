    ...
    return (
        <>
            ...
            <section className="sr-only">
                <h2>{displayName} AI 기술적 분석 — 보조지표 및 캔들 패턴</h2>
                <p>
                    {displayName}({ticker}) 종목의 실시간 차트를{' '}
                    {INDICATOR_KIND_COUNT}종 보조지표로 자동 분석합니다.
                    RSI(상대강도지수), MACD(이동평균수렴확산), 볼린저밴드,
                    단순이동평균(SMA), 지수이동평균(EMA), 스토캐스틱,
                    ATR(평균진폭), OBV(거래량균형지수), CCI(상품채널지수),
                    파라볼릭SAR, 슈퍼트렌드, 켈트너채널,
                    VWAP(거래량가중평균가격) 등 {INDICATOR_KIND_COUNT}종 지표를
                    분석합니다.
                </p>
                <p>
                    {CANDLESTICK_SKILL_COUNT}종 캔들 패턴 분석: 도지(Doji),
                    해머(Hammer), 역망치(Inverted Hammer), 교수형(Hanging Man),
                    유성형(Shooting Star), 불리시 인겔핑(Bullish Engulfing),
                    베어리시 인겔핑(Bearish Engulfing) 등 주요 캔들 패턴을 자동
                    감지합니다.
                </p>
                <p>
