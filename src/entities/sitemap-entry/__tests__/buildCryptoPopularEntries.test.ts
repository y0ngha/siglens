...
describe('buildCryptoPopularEntries', () => {
    ...
    it('emits chart + news + fear-greed + overall + position per crypto (no stock-only routes)', () => {
        ...
        // Pin the exact count: 5 routes per coin — the full CRYPTO_DESCRIPTOR.tabs set
        // (root, /news, /fear-greed, /overall, /position).
        ...
        expect(paths).toContain('/BTCUSD/position');
    ...
    it('emits exactly POPULAR_CRYPTOS.length × 5 total entries', () => {
        const entries = buildCryptoPopularEntries(now);
        expect(entries).toHaveLength(POPULAR_CRYPTOS.length * 5);
    });
    ...
    it('chart/fear-greed use daily changeFrequency; overall uses weekly', () => {
        ...
        const positionEntry = entries.find(
            e => e.url === 'https://siglens.io/BTCUSD/position'
        );
        ...
        expect(positionEntry?.changeFrequency).toBe('daily');
        expect(positionEntry?.priority).toBe(0.7);
        // 다른 6h-boundary 탭과 같은 lastmod 계열이어야 한다(rolling 금지).
        expect(positionEntry?.lastModified.toISOString()).toBe(
            chartEntry?.lastModified.toISOString()
        );
    });
