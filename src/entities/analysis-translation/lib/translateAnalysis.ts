 ...
 * **산문만** 저가 모델로 옮긴다 — 숫자 필드와 enum은 `*Ko` 후보가 아니라
 * 구조적으로 제외된다. 다만 `priceRangeKo`(`'190~200달러'`)처럼 **문자열 안에
 * 든 숫자**는 넘어가며, 그 보존은 구조가 아니라 프롬프트 규칙이 맡는다
 * (`api.ts`의 "Never change numbers, prices, percentages…").
 *
