 ...
 * 산문 화이트리스트에 있어 그대로 두면 **이미 카탈로그에서 번역해 온 문구를 LLM에
 * 다시 보낸다** — 프롬프트가 "Translate each Korean sentence"라 일본어를 넣으면
 * 결과가 망가지고, 게이트 거부마다 DeepSeek 왕복이 붙는다.
 *
 * 라우트의 조기 반환(`withLocalizedProse`)이 그걸 막는데, 그 분기에는 테스트가
 * 전혀 없었다 — 가드를 지워도 10,516개가 전부 초록이었다(실측). 여기서
 * **추출 단계의 사실**을 고정해 왜 그 분기가 필요한지 못 박는다.
 */
