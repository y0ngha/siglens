 ...
 *
 * 업스트림(`guardPlainText`)은 길이의 상한도 하한도 두지 않는다 — 상한은 분석
 * 타입마다 압축률이 37~105%로 제각각이라 고정값이 의미가 없고, 하한은 원본보기
 * 토글이 항상 옆에 있어 짧은 재작성이 내용을 숨기지 않으므로 없앴다. 따라서 이
 * 값이 파이프라인 전체에서 유일한 길이 방어선이다:
