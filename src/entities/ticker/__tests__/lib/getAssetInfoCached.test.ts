...
vi.mock('../../actions/getAssetInfoAction', () => ({
...
const mockGetAssetInfoAction = getAssetInfoAction as MockedFunction<
...
describe('getAssetInfoCached', () => {
    ...
    it('자체 cache 처리 없이 매 호출이 underlying action으로 위임된다 (vitest 환경엔 React render pass 없음)', async () => {
        mockGetAssetInfoAction.mockResolvedValue(null);
        ...
        // vitest 환경(non-RSC)에선 두 호출이 모두 underlying action을 통과한다.
