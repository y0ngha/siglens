...
describe('syncKrListedTickers', () => {
     ...
     * 기능은 통째로 죽는다.
     *
     * 여기서 검증하는 것은 **그 상황이 왔을 때 sync 래퍼가 어떻게 끝나는가**이지, SQL
     * 필터 자체가 아니다(이 스위트는 리포지토리를 통째로 mock한다). 필터가 실제로 걸리는지는
     * `api.test.ts`의 `findAllListingStatuses` 케이스가 조건식을 열어 확인한다.
     */
