...
4. **cron이 최소 2번의 완전한 밤(≥2 full nights) 동안 돌게 두고** 행이 쌓이는지 확인한다:
   ...
   ```
   유니버스는 290개 심볼 / 1913개 심볼×탭 조합이다. 공정 선별(위 §"공정 선별")+콜드 캐시 폴링(FIX Z) 적용 후 steady state에서는 하룻밤에 유니버스 전체를 커버할 수 있어야 한다(정확한 실측치는 위 참고).
5. Phase 2를 배포한다.
