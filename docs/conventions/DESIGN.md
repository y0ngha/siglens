...
**크롬은 뷰포트에, 본문은 읽기 폭에** 맞춘다.

| 층 | 대상 | 클래스 | 값 |
|---|---|---|---|
| 크롬 | 사이트 헤더 · 푸터 | 전폭 `px-4` | 뷰포트 기준 16px |
...
안쪽으로 들여쓰여 두 줄이 어긋났다 — 1920px에서 448px. 제품의 주 표면이 심볼 페이지이므로
크롬을 뷰포트에 맞추는 쪽을 골랐다. 그 결과 `page-container` 라우트에서는 헤더 로고가
본문보다 바깥에서 시작하는데, GitHub이 쓰는 것과 같은 배치(크롬 전폭 · 본문 중앙)다.

크롬 값을 바꾸면 **짝이 되는 자리를 전부 같이 본다** — 사이트 헤더, 푸터, 심볼 헤더,
심볼 헤더 스켈레톤, 탭 레일, 탭 스켈레톤, 차트 제목 줄까지 일곱 곳이 같은 규약을 공유한다.

### 푸터의 두 축

전폭이 되면 링크를 가운데 모아둘 이유가 없어지는데, 사이트맵만 왼쪽에 붙이면 넓은 화면에서
오른쪽 절반이 통째로 빈다. 그래서 한 가로 행의 양 끝에 **성격이 다른 두 묶음**을 붙인다 —
왼쪽은 "누가 만들었고 어떤 약속을 하는가"(저작권·저장소·약관·문의), 오른쪽은 "어디로 갈 수
있는가"(사이트맵). 면책 고지는 어느 축도 아니라 구분선 아래 자기 줄을 갖는다.
랜드마크도 같은 축으로 갈라 둔다(`사이트 정보` / `사이트맵`).

푸터 링크는 **전 페이지에 렌더되는 전역 링크**라 앵커 텍스트를 바꾸면 사정거리가 사이트
전체다. 카테고리 열로 묶으며 보이는 글자를 짧은 라벨로 줄였지만 DOM 텍스트는 `sr-only`
조각으로 `fullLabel`을 유지한다. 접근성 이름은 별도로 `aria-label`에 못박는데,
`computeAccessibleName`이 텍스트 노드를 **각각 trim한 뒤 이어붙여** 조각 사이 공백을
먹기 때문이다(`미국` + `sr-only(" 시장 분석")` → `미국시장 분석`).
