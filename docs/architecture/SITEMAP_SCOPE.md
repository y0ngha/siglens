...
4. **그 다음에** `POPULAR_TICKERS`와 `korea-equity` 카테고리에 함께 추가한다.
   두 목록은 테스트로 동기가 강제된다(`popular-tickers.test.ts`) — 카테고리 그리드가
   한국 종목으로 가는 **유일한 크롤 가능한 링크**라, 여기 빠지면 sitemap에만 있는 고아가 된다.
