# Sitemap 범위 — 무엇을 색인에 올릴지 정하는 규칙

이 문서는 "왜 sitemap에 종목이 이것뿐인가"에 답한다. 특히 **한국 종목 마스터에는
2,595종목이 시드돼 있는데 sitemap에는 20종목만 실려 있다** — 그건 미완성이 아니라 결정이다.

## 1. 결정: 시드된 전 종목을 sitemap에 넣지 않는다

**대상**: `korean_tickers`에 시드된 KOSPI·KOSDAQ 2,595종목(2026-08 기준).
**결정**: sitemap에는 큐레이션된 20종목만 싣는다. 나머지는 색인 대상이 아니다
(`evaluateSymbolIndexability`가 화이트리스트 밖 심볼을 longtail-default-blocked로 판정한다).

### 근거 — 이 저장소는 이미 한 번 해봤고 대가를 치렀다

2026-07에 노출이 절벽처럼 꺾였다. 원인은 두 가지가 겹친 것이었고, 그중 우리가 만든 쪽이
**thin 콘텐츠 대량 색인**이다. 그때의 대응이 지금 코드에 그대로 남아 있다:

- `sitemap-longtail-:page.xml`은 **410 Gone으로 은퇴**했다
  (`src/app/api/sitemap/longtail/[page]/route.ts`). 라우트를 지우지 않고 410을 반환하는
  이유는 이미 색인된 URL을 크롤러에게 명시적으로 회수시키기 위해서다.
- `sitemap-removal-:kind.xml` 계열이 신설됐다 — 색인에서 **빼기 위한** sitemap이다.
- sitemap index는 5개에서 3개(static/popular/crypto)로 줄었다.

2,595종목 × 6탭 = **15,570 URL**은 은퇴시킨 longtail sitemap보다 큰 규모다. 같은 실수를
이름만 바꿔 다시 하는 것이다.

### 근거 — 종목이 존재한다는 것과 페이지에 내용이 있다는 것은 다르다

큐레이션 20종목은 "yahoo가 시세를 준다"가 아니라 **차트·펀더멘털·재무가 모두 채워지는 것을
실측으로 확인한** 종목들이다(`popular-tickers.ts` KR 블록 주석). 코스닥 하위 종목은 상당수가
재무 데이터가 비거나 뉴스가 잡히지 않아, 페이지는 200으로 뜨지만 내용은 폴백 문구다.
색인에 올릴 가치가 있는 페이지와 렌더 가능한 페이지는 같은 집합이 아니다.

이 판단은 미국 주식·암호화폐에도 똑같이 적용돼 있다 — FMP는 수천 종목을 주지만
`POPULAR_TICKERS`와 `POPULAR_CRYPTOS`만 sitemap에 실린다. 한국 종목만 다르게 대할 이유가 없다.

## 2. 늘리려면 — 순서가 정해져 있다

큐레이션 목록은 고정이 아니다. 다만 **sitemap이 먼저가 아니라 마지막**이다.

1. **prewarm 유니버스에 넣는다** (`POPULAR_TICKERS`). 그래야 `seo_analysis_snapshots`에
   봇이 읽을 서술이 생긴다. 이게 실질 비용이다 — 심볼당 탭 5개의 LLM 생성이 매일 밤 돈다.
2. **용량을 확인한다.** 밤당 처리량은 대략 `틱 수 × SYMBOLS_PER_TICK`이고, 현재 유니버스
   대비 여유가 얼마인지는 `[seo-prewarm] batch done` 로그의 `staleTotal`/`remaining`으로
   본다. `remaining`이 매일 밤 0으로 수렴하지 않으면 이미 포화다 — 그 상태에서 심볼을
   더하면 기존 종목의 스냅샷이 낡는다.
3. **콘텐츠 깊이를 실측한다.** 봇이 받는 SSR 본문 길이를 잰다
   (`e2e/specs/kr-equity-seo.spec.ts`의 thin 콘텐츠 가드가 같은 측정을 자동화한다).
   2026-07 절벽 당시 봇이 받던 양이 677자였다. 그 근처면 넣지 않는다.
4. **그 다음에** `POPULAR_TICKERS`와 여섯 개 `kr-*` 카테고리 중 해당 업종
   (`kr-semiconductor` / `kr-auto-battery` / `kr-bio` / `kr-platform` / `kr-finance` /
   `kr-kosdaq`)에 함께 추가한다. 불변식은 **합집합**이다 — `kr-*` 카테고리 종목의
   합집합 = `POPULAR_TICKERS`의 KR 블록이고, 한 종목이 두 카테고리에 들어가면 안 된다
   (`popular-tickers.test.ts`가 둘 다 강제). 카테고리 그리드가 한국 종목으로 가는
   **유일한 크롤 가능한 링크**라, 여기 빠지면 sitemap에만 있는 고아가 된다.

   > 2026-08 이전에는 `korea-equity` 단일 카테고리였다. 업종 분할 후에도 검사 대상은
   > 개별 카테고리가 아니라 합집합이라는 점이 핵심이다 — 카테고리를 더 쪼개도 한 종목이
   > 어느 카드에도 안 들어가면 같은 고아가 생긴다.

## 3. 자동화한다면 — 게이트는 "종목 존재"가 아니라 "스냅샷 존재"여야 한다

목록을 손으로 관리하는 게 언젠가 부담이 되면, 자동화의 게이트는 반드시
**`seo_analysis_snapshots`에 fresh 행이 있는 심볼만 sitemap에 싣는다**여야 한다.
`korean_tickers` 멤버십을 게이트로 쓰면 그건 2,595종목을 넣는 것과 같다.

이 방향은 인과가 옳다: 유니버스(편집 결정 + LLM 예산)가 앞서고 sitemap이 따라온다.
반대로 하면 sitemap이 먼저 커지고 콘텐츠가 못 따라가면서 정확히 2026-07이 재현된다.

## 3-1. 예외가 아니라 §2의 적용 — `/market` 섹터 허브 링크 대상 (2026-08-24)

색인된 허브가 링크하는 심볼은 큐레이션 목록에 있어야 한다. 이건 §1의 "롱테일을 열지
않는다"와 충돌하지 않는다 — 롱테일은 **아무도 링크하지 않는** 27,000종목이고, 여기 대상은
우리가 손으로 고른 섹터 그리드가 이미 크롤 가능하게 링크하고 있던 것들이다.

**실측(2026-08-24, 프로덕션 Googlebot UA)**: `SECTOR_ETFS`(11) + `SECTOR_STOCKS`(80) 중
23종이 `POPULAR_TICKERS`에 없어 `longtail-default-blocked`였다. 23종 전부
`200` + `noindex, nofollow` + 본문 893~1,131자. 즉 **색인된 허브가 색인 불가한 막다른
페이지 23개를 링크**하고 있었다 — 크롤 예산은 소모되고, 허브의 `ItemList` JSON-LD가
가리키는 대상은 색인될 수 없다.

§2 순서를 그대로 밟았다:

1. **prewarm 유니버스** — `POPULAR_TICKERS`에 추가해 `buildPrewarmUniverse()`가 집는다.
2. **용량** — 유니버스 312 → 335, 유닛 2,027 → 2,165. 야간 처리량은 틱 약 90회 ×
   `SYMBOLS_PER_TICK` 6 ≈ 540 심볼-슬롯이라 여유가 있다. `[seo-prewarm] batch done`의
   `remaining`이 매일 밤 0으로 수렴하는지 계속 본다.
3. **콘텐츠 깊이** — 전원 SPDR 섹터 ETF 또는 S&P 500 대형주로, 기존 283종과 같은 FMP
   파이프라인에서 차트·펀더멘털·재무가 채워진다. 2026-07 절벽 당시 기준(677자) 근처가
   아니다.
4. **목록 반영** — `POPULAR_TICKERS` `[13]` 블록. sitemap은 `buildPopularEntries`가
   자동으로 따라오되, 섹터 ETF 11종은 `KNOWN_ETF_TICKERS`에 있어 `/financials`
   엔트리가 나가지 않고(재무제표 없음 → 영구 noindex), `/options`는
   `POPULAR_OPTIONS_TICKERS` 게이트가 막는다.

**불변식**: `SECTOR_ETFS ∪ SECTOR_STOCKS ⊆ POPULAR_TICKERS`.
`src/shared/config/__tests__/marketHubIndexability.test.ts`가 멤버십이 아니라
`evaluateSymbolIndexability`의 판정으로 강제한다. dashboard 그리드에 심볼을 더하면서
여기를 안 고치면 같은 결함이 조용히 재발한다.

## 3-2. GSC 실측 수요 기반 확장 96종 (2026-08-24)

§1은 "롱테일을 열지 않는다"고 정한다. 이 확장은 그 원칙의 예외가 아니라 **§2가 요구하는
근거를 실제로 갖춘 목록**이다 — 아무도 링크하지 않는 27,000종목을 여는 것이 아니라,
Google이 이미 노출시켰던 심볼 중 상위 96종을 실측으로 골라 넣는다.

### 선정 방법 (전부 실측)

1. GSC `실적 > 페이지` 90일 상위 1,000행 → 심볼 850종 집계.
2. 화이트리스트 밖 상위 96종에 대해 프로덕션 Googlebot UA로 전수 측정:
   - `/{sym}` 봇 SSR 길이 → 콘텐츠 깊이
   - `/{sym}/fundamental`·`/financials` SSR 길이 → 데이터 가용성
     (코드가 이미 `profile === null`·`isEmptyFinancialsSnapshot`을 noindex 게이트로 쓴다)
   - 같은 페이지가 렌더하는 시가총액 → 규모
3. 등급을 나눠 `popular-tickers.ts` `[14]` 블록에 노출 내림차순으로 배치.

### 실측 결과와 그 함의

**후보 96종 전부 645~1,239자다.** 색인 중인 화이트리스트는 2,399~3,726자(중앙값 3,042),
2026-07 절벽 당시 봇이 받던 양은 677자였다. 즉 지금 이 심볼들은 절벽 당시 수준에 가깝다.

이건 "그래서 넣으면 안 된다"가 아니라 **닭-달걀**이다. 얇은 이유는 AI 스냅샷이 없어서고,
스냅샷은 prewarm 유니버스에 있어야 생긴다. §2가 "sitemap이 먼저가 아니라 마지막"이라고
정한 이유가 이것이다 — 유니버스에 넣고, 스냅샷이 붙고, 그때 색인된다. 스냅샷이 붙기
전까지는 `hasSnapshot` 게이트가 degraded 페이지를 계속 막는다.

### 등급과 리스크

| 등급 | 종수 | 90일 노출 | 성격 |
|---|---|---|---|
| A (ETF) | 16 | 1,349 | 미국 정규 상장 ETF. SOXX·TLT·QQQM·SCHD·SLV 등 |
| A (주식) | 13 | 896 | 미국 정규 상장 중대형주. PEN·BAH·CNC·ARW·AVT 등 |
| B | 10 | 786 | OTC/ADR. 에어리퀴드·스미토모금속광산·하마마츠포토닉스 등 |
| C | 57 | **4,372** | 페니주·SPAC·워런트·뮤추얼펀드·2배 단일종목 ETF |

**C 구간은 2026-07 절벽에서 지목된 종목군과 형태가 같다.** 그때 GSC 색인 목록을 최종크롤
오름차순으로 뒤집었을 때 나온 것이 PSGCX(뮤추얼펀드)·BPYPO(우선주)·STTDF/LKHLY(OTC/ADR)·
TSDD(레버리지ETF)·ALORU(SPAC유닛)였고, C 구간이 정확히 그 구성이다. 동시에 **실측 수요가
가장 큰 구간**이기도 하다(노출 1위 QSPT 577이 여기 있다). 이 둘은 같은 사실의 양면이다 —
그때 노출을 만들던 것이 그때 벌을 받은 것과 같은 페이지다.

색인 가치가 있는지는 코드가 아니라 관측으로만 갈린다. 이 확장은 그 관측을 하기 위한
것이고, 되돌릴 수 있게 설계했다.

### 용량 (반드시 확인)

유니버스 335 → **431**, 유닛 2,165 → **2,741**(+27% LLM 호출). 야간 처리량은
틱 약 90회 × `SYMBOLS_PER_TICK` 6 ≈ 540 심볼-슬롯이므로 이론 가동률이 62% → **80%**다.
실제 처리량은 in-flight 점유와 터미널 6h 백오프 때문에 이론보다 낮다.

**배포 후 `[seo-prewarm] batch done` 로그의 `remaining`이 매일 밤 0으로 수렴하는지
확인한다.** 수렴하지 않으면 이미 포화이고, 그 상태에서는 새로 넣은 심볼이 기존 종목의
스냅샷을 낡게 만든다(2026-08-18에 KR 대장주 3종이 회전 순번에 도달하지 못해 스냅샷 0행이
된 전례가 있다). 그때의 선택지는 `SYMBOLS_PER_TICK` 상향 또는 목록 축소다.

### 롤백 순서

`[14]` 블록은 **노출 내림차순**이라 뒤에서부터 자르면 된다. 롤백 신호는 두 가지:
GSC `크롤링됨 - 현재 색인이 생성되지 않음`이 늘거나, 사이트 평균 게재순위가 나빠지는 것.
그 경우 C → B → A 순으로 되돌린다.

## 4. 상장폐지

`korean_tickers.delisted_at`이 상장 상태를 들고 있고, 일 1회 크론(`docs/reference/CRON.md`
`kr-tickers`)이 갱신한다. 검색은 상장 중인 행만 본다.

**`POPULAR_TICKERS`는 하드코딩이라 자동으로 빠지지 않는다.** 그 목록의 종목이 폐지되면
크론이 `[kr-tickers] delisted popular ticker — remove from POPULAR_TICKERS: …`를 error로
남긴다. 사람이 목록에서 빼기 전까지 sitemap은 그 URL을 계속 싣는다.

### 4-1. 런타임 콘텐츠 게이트 (2026-08-24)

sitemap은 여전히 그 URL을 싣지만, **차트 라우트가 런타임에 noindex로 막는다.**
`evaluateSymbolIndexability`의 `hasPriceData` 입력이 `false`면 화이트리스트 여부와
무관하게 `no-price-data`로 차단된다.

**발견 경위**: 2026-08-24 유니버스 431종 전수 fetch에서 14종
(ASSF·BSTG·CLAA·CTK·DIRV·DYFN·ENTF·ESMT·GTS·HVBC·PSMC·SRYB·SWAR·TLIIX)이 봉 데이터가
없어 차트 페이지가 제목 + sr-only 개요만 남은 **고유 330자 껍데기**였는데, 전부
`index, follow`였다. 대부분 상장폐지·비상장 전환된 미국 티커다(위 크론은 KR만 본다).
게이트가 멤버십만 보고 "지금 이 페이지에 콘텐츠가 있는가"를 보지 않았기 때문이다.

**왜 목록에서 지우지 않고 런타임 신호로 두는가** — 양방향 자가치유다. 새로 폐지되는
종목은 자동으로 빠지고, 데이터 공급이 일시 중단됐다 복구된 종목은 자동으로 돌아온다.
손으로 지우면 후자가 영구히 사라진다. 대가는 GSC "제출된 URL이 noindex로 표시됨" 경고가
그 수만큼 뜨는 것인데, 목록이 14종 규모이고 데이터가 복구되면 스스로 사라진다.

**술어는 본문과 같아야 한다.** 처음에 `bars.length > 0`으로 두었더니 CTK(봉 1개)가
새어 나갔다 — 본문의 지표 요약 블록은 `buildTechnicalFacts`가 non-null일 때만 렌더되고,
그 헬퍼는 등락률 분모로 직전 봉이 필요해 2봉 미만이면 null이다. 지금은 게이트도 같은
헬퍼로 판정한다.

**조회 실패는 차단하지 않는다.** 봉 조회가 throw하면 `hasPriceData`를 `undefined`로
남겨 기존 판정을 따른다. 실패를 `false`로 매핑하면 FMP 장애 한 번이 전 종목 색인 해제로
번진다.

## 4-2. 내부링크 — sitemap만으로는 부족하다 (2026-08-24)

sitemap 등재와 **내부링크 확보는 별개**다. 같은 날 실측에서 sitemap이 광고하는 431종 중
내부링크를 하나라도 받는 것은 **128종뿐**이었다(홈 94 + `/market` 50 + `/market/kr` 16의
합집합). 심볼 페이지끼리는 서로를 0개 링크했다 — 탭 바는 같은 심볼의 다른 탭만 잇는다.
나머지 **303종(70%)은 XML sitemap 외 진입로가 없었다.**

같은 시점 GSC 28일: 차트 탭이 노출의 48%(182/376)를 받으면서 평균 순위 **44.7위**였다.
형제 탭(fundamental 7.0 / news 6.7 / overall 5.0 / fear-greed 3.2)과 6~10배 차이다.

**조치**: 차트 라우트 본문에 "관련 종목" 칩 스트립을 둔다(`shared/config/relatedSymbols.ts`
+ `views/symbol/RelatedSymbols.tsx`). 심볼당 8개, 관련성 우선(테마 피어 → 링 이웃)이되
**링 ±1은 반드시 포함**해 유니버스를 하나의 순환으로 잇는다 — 고아 0이 여기서
산술적으로 보장된다. 섹터/테마 데이터만으로 짜면 커버리지가 146/402라 나머지 256종이
다시 고아가 되므로 링 보장을 뺄 수 없다(`relatedSymbols.test.ts`가 고정, 변이 검증 완료).

**목록형 디렉터리 페이지는 만들지 않았다** — 431종을 한 페이지에 나열하면 링크당 가치가
희석되고 doorway 성격이 강해진다. 사용자 결정.

**시장을 넘는 링크는 금지가 아니라 근거의 문제다.** 링(배열 인접성)은 위치 잡음이라
시장을 넘을 근거가 못 되지만(합쳐 두었을 때 `AAPL` 옆에 `HPSP`가 붙었다), 사람이 판단한
테마는 근거가 된다. `CROSS_MARKET_THEME_GROUPS`가 미국·한국을 의도적으로 한 그룹에
묶는다 — AI 반도체 밸류체인(`NVDA ↔ 삼성전자·SK하이닉스`), 전기차·2차전지
(`TSLA ↔ 현대차·LG에너지솔루션`), 인터넷 플랫폼(`GOOGL ↔ 네이버·카카오`), 바이오
(`LLY ↔ 삼성바이오로직스·셀트리온`). 한국 사용자가 `NVDA 주가`를 검색한 뒤 다음으로
보는 것이 대개 그쪽이고, 산업 관계도 실제로 그렇다.

여러 테마에 속한 심볼은 **라운드로빈**으로 섞는다. 평탄화하면 먼저 나열된 그룹이 예산
6칸을 독식한다 — `NVDA`는 메가캡 카테고리(피어 8개)와 AI 반도체 두 그룹에 속하는데,
순서대로 자르면 삼성전자·SK하이닉스가 한 칸도 못 들어간다(변이 검증으로 확인: 라운드로빈을
빼면 교차시장 테스트 6건이 전부 실패한다).

## 5. lastmod

`buildPopularEntries`가 종목이 상장된 거래소의 마지막 마감을 `lastmod`로 쓴다 —
국내 종목은 KRX(15:30 KST), 나머지는 NYSE(16:00 ET, 반장은 13:00). 계산은
`shared/lib/marketSessionDate.ts`가 전담하며 주말·DST·NYSE 휴장일을 모두 처리한다.

`MarketProfileDescriptor.sitemapLastmod`라는 필드가 한때 이 등급을 선언했지만 읽는 곳이
없었다(2026-08 제거). 심볼→프로필 판정이 어차피 호출부에 필요해서 그 필드를 배선해도
분기가 줄지 않았고, 선언만 있고 강제되지 않는 설정은 조용히 실제 동작과 어긋난다.
