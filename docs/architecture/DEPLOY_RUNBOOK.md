...
> #                                 [fear-greed us/kr/crypto + market-kr 통합], config-signal
...
| `siglens-market-data-loader-failed` | `[FearGreedRoute] getMarketFearGreedStatic failed` 또는 `[FearGreedKrRoute] getMarketFearGreedKrStatic failed` 또는 `[FearGreedCryptoRoute] getMarketFearGreedCryptoStatic failed` 또는 `[MarketContent:kr]` 1시간 합계 > 4가 연속 2주기 (2026-09-14 통합 — 옛 `siglens-fear-greed-loader-failed`/`-kr-loader-failed`/`-market-kr-loader-failed`) | 넷 다 fail-open 설계라 **이게 유일한 신호다**. `/fear-greed`는 200 + "표본이 부족합니다"를 렌더하고 그 HTML이 ISR/S3에 저장돼 5xx도 헬스체크 실패도 안 뜬다(FMP 402/403이면 빈 페이지가 영구화). `/fear-greed/kr`은 같은 구조에 **무인증 yahoo** 429가 주 원인. `/fear-greed/crypto`는 US와 같은 FMP 구조(21개 심볼). `/market/kr`은 빈 배열로 fail-open해 canonical null + noindex가 굳는다. **어느 라우트인지는 Logs Insights로 원문을 봐야 안다** — `fields @timestamp, @message \| filter @message like /getMarketFearGreedStatic failed\|getMarketFearGreedKrStatic failed\|getMarketFearGreedCryptoStatic failed\|MarketContent:kr/ \| sort @timestamp desc`. FMP 키/플랜(SSM `/siglens/FMP_API_KEY`) 또는 `marketFearGreedKrSymbols.ts`·`marketFearGreedCryptoSymbols.ts` 확인. 복구 후 다음 revalidate(최대 1h)에 자동 정상화 |
