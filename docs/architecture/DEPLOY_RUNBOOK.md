...
3. **SSM 파라미터** — `DEEPSEEK_CHAT_API_KEY`(필수, 없으면 첫 요청부터 실패),
   `BRAVE_SEARCH_API_KEY`(선택 — 범용 웹. 이미 있는 `NAVER_CLIENT_ID/SECRET`이
   한국어 질의의 뉴스 절반을 맡으므로 둘 중 하나만 있어도 `web_search`가 켜지고,
   둘 다 없을 때만 툴이 가용 목록에서 빠진다. 네이버만 있으면 영어 질의는 0건),
