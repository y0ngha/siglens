...
| `/terms` · `/privacy` | **86400 (24h)** | **재검증이 없으면 안 되는 페이지다.** 본문이 코드가 아니라 `terms` 테이블에 있고, 발효일 전환(`effective_date <= NOW()`)과 번역 적재(`content_translations`)가 전부 **렌더 시점에만** 평가된다. v0.69.0에서 이 export가 없어 `s-maxage=31536000`으로 굳었고, 개인정보처리방침 v2와 en/ja/zh 번역이 DB에 있는데도 화면에 안 나왔다. 하루면 충분하다 — 약관 개정은 사전 고지를 거치므로 시각 단위 정확도가 필요 없다 |
