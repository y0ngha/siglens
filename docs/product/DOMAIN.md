...
분석 도메인 로직의 단일 진실 공급원은 `@y0ngha/siglens-core`다.
이 문서는 core가 제공하는 타입, 계산 규칙, 프롬프트/Skills 계약을 정리한다.
siglens 앱 레포에는 FSD 구조에 따라 UI, Server Action, 데이터 어댑터, 표현 계층 보조 함수만 둔다.

순수 계산 로직은 외부 의존성이 없어야 한다. 데이터 fetch, SDK 호출, 파일 I/O는 siglens 앱의
`entities/`, `features`, `shared` 어댑터나 core의 infrastructure 계층에 둔다.
...
```typescript
// @y0ngha/siglens-core public types
...
export interface Bar {
    ...
    close: number;
    volume: number;
    vwap?: number;  // provider가 제공하는 경우에만 사용
}
...
```typescript
// @y0ngha/siglens-core market constants
...
```typescript
// @y0ngha/siglens-core indicator constants
...
6. Premium / Equilibrium / Discount Zones
   - 최근 구조 이탈(BOS/CHoCH)을 앵커로 range 를 산출 (resolveZoneRange)
     a. Bullish break 이후:
        - top    = max(이탈 가격, 이탈 시점 이후 형성된 swing high 가격들)
        - bottom = min(이탈 시점 이후 형성된 swing low 가격들)
                   (post-break low 가 없으면 이탈 이전 swing low 로 fallback)
     b. Bearish break 이후:
        - top    = max(이탈 시점 이후 형성된 swing high 가격들)
                   (post-break high 가 없으면 이탈 이전 swing high 로 fallback)
        - bottom = min(이탈 가격, 이탈 시점 이후 형성된 swing low 가격들)
     c. 구조 이탈 미발생:
        - top    = max(전체 swingHighs 가격)
        - bottom = min(전체 swingLows 가격)
   - range = top − bottom
   - Premium zone:     [top, bottom + 0.75 × range]               (상단 25%)
   - Equilibrium zone: [bottom + 0.75 × range, bottom + 0.25 × range]  (중간 50%)
   - Discount zone:    [bottom + 0.25 × range, bottom]            (하단 25%)
   - range ≤ 0 이거나 앵커 해석 실패 시 세 zone 모두 null
...
```typescript
// @y0ngha/siglens-core indicator exports
...
`@y0ngha/siglens-core`의 key-level 검증 로직 — 순수 함수로 구현, 외부 의존성 없음.
...
`@y0ngha/siglens-core`의 candle analysis 로직 — 순수 함수로 구현, 외부 의존성 없음.
...
// timeframe 은 PromptConfig (trendSampleCount / recentBarsCount /
// squeezeZeroCrossLookback) 를 해석하는 데 사용된다. 자세한 맵은
// prompt.ts 의 resolvePromptConfig 참조.
```
...
**파일 읽기 책임**: siglens 앱의 `entities/skill/api.ts`. core prompt builder는 파싱된 `Skill[]`만 받으며 파일 I/O를 하지 않는다.
...
siglens 앱이 파싱한 skill 파일을 core prompt builder에 전달할 때 사용하는 타입.
core는 이 타입만 받고 파일 경로나 원본 Markdown은 알지 못한다.
...
entities/llm-provider/api/*
  → 완성된 프롬프트로 선택된 AI provider 호출
...
core prompt builder와 `entities/llm-provider/api/*`를 수정할 때는 다음 원칙을 준수한다.
...
| 위치 | 역할 |
|---|---|
| `AI_SYSTEM_PROMPT` (core 또는 `entities/llm-provider/lib` 공통 유틸) | 페르소나, 결정성, anti-fabrication, 출력 형식, 한국어·존댓말 규칙 — 모든 호출에 공통인 행동 규약 |
...
- `entities/skill/api.ts`의 `readdir`는 OS·inode 순서를 보장하지 않는다.
...
- core prompt builder의 섹션 순서를 변경할 때는 해당 core 테스트의
  position assertion(예: Analysis Guidelines → Analysis Request 순서) 이 깨지지 않는지 확인한다.
- AI provider 호출 옵션을 변경할 때는 `src/entities/llm-provider/__tests__/`의
  spy 검증을 함께 갱신한다.
...
siglens 레포의 `entities/options-chain`과 `widgets/options`는 데이터 어댑터, Server Action, UI 계층 보조 헬퍼만 보관한다.

### 위치

`src/entities/options-chain/` — yahoo-finance2 어댑터, 옵션 체인 Server Action, 표현 계층 선택 헬퍼.
`src/widgets/options/` — 옵션 페이지 UI, React hooks, 차트/테이블 전용 유틸.

```
src/entities/options-chain/
├── actions/optionsActions.ts
├── lib/YahooOptionsAdapter.ts
├── lib/findNearestStrike.ts
├── lib/pickActiveChain.ts
├── lib/optionsDataCache.ts
└── lib/yahooNormalize.ts
```

### 타입

```typescript
// src/entities/options-chain/lib/types.ts
...
```typescript
// src/entities/options-chain/lib/findNearestStrike.ts
...
```typescript
// src/entities/options-chain/lib/pickActiveChain.ts
...
LLM 호출과 모델 선택의 핵심 타입 / 상수는 `@y0ngha/siglens-core`와 `src/shared/config/llmProviders.ts`를 기준으로 한다.
실제 provider SDK 호출은 `src/entities/llm-provider/api/`에서 담당하며, 사용자 API 키 저장/검증은 `entities/api-key`가 소유한다.

```
src/entities/llm-provider/
├── api/                # Anthropic / Gemini / OpenAI adapter
├── lib/                # JSON parse, provider defaults, utilities
└── index.ts            # slice public exports
...
- 지원 provider: Anthropic, Google, OpenAI (`@y0ngha/siglens-core` provider 타입 + `src/shared/config/llmProviders.ts`)
- 키 저장: `LLM_API_KEY_ENCRYPTION_KEY` 로 암호화하여 DB에 저장
- 등록/조회/삭제: `src/entities/api-key/actions/` 의 Server Action
- core 정책은 키 자체를 보지 않으며, provider 식별자만 다룬다

---

## Free Analysis Tier Gating

`@y0ngha/siglens-core` 0.35.0부터 기술 분석 응답 자체에 timeframe·정보 깊이 게이팅을
적용한다. core가 게이팅 정책의 단일 진실 공급원이며, 앱은 비활성 컨트롤·잠금 티저 같은
사용성 계층만 담당한다.
...
type TierInfoDepth =
    | 'direction' | 'summary' | 'partial_detail' | 'full_detail'
    | 'entry' | 'stoploss' | 'target' | 'confidence';

interface FilteredAnalysisResult {
    result: AnalysisResponse;
    /** tier가 허용하지 않아 result에서 제거된 TierInfoDepth 조각 목록. */
    lockedInfoDepth: readonly TierInfoDepth[];
}
```

### TIER_CONFIG / 게이팅 함수 (core exports)

- `TIER_CONFIG.timeframes`: tier별 허용 timeframe. `free`는 `1Day`만, `member`/`pro`는
  분·시간을 포함한 전체 등록 timeframe
- `TIER_CONFIG.infoDepth`: tier별 허용 정보 깊이. `free`는 `direction`(trend)과
  `summary`만, `member`/`pro`는 `partial_detail`/`entry`/`stoploss`/`target`/
  `full_detail`/`confidence`를 포함한 전체 깊이
- `isTimeframeAllowed(tier, timeframe)`: 제출·캐시읽기·정적 peek·폴링 네 경로가 모두
  공유하는 단일 timeframe 게이트
- `filterAnalysisResult(result, tier, config?)`: 원본 분석에서 tier가 허용하지 않는
  필드를 `null`로 치환하고, 제거된 필드 목록을 담은 `FilteredAnalysisResult`
  (`result` + `lockedInfoDepth`)를 반환한다
...
- `src/views/symbol/hooks/useAnalysis.ts`: `lockedInfoDepth`를 상태로 보관한다.
  ...
  응답을 받으면 안전 기본값(전체 잠금)으로 대체하고, member/pro 호출자가 같은 구버전
  응답에서 `lockedInfoDepth` 필드 자체가 없으면 빈 배열로 기본값 처리한다.
- `src/widgets/analysis/AnalysisPanel.tsx`: `lockedInfoDepth.length > 0`만으로
  리스크 배지·매매 전략·주요 레벨을 SSR부터 숨기고 회원가입 유도 티저를 그 자리에
  렌더한다. tier hydration 완료 여부에는 의존하지 않으므로, 크롤러를 포함한 첫 SSR
  페인트에서도 실제 값을 노출하지 않는다.

---

## Signal Detection

Market signal detection logic is exported by `@y0ngha/siglens-core` and consumed by the `/market` dashboard.
