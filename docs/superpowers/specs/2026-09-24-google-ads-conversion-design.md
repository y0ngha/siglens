...
- `effectiveDate: 2026-10-05T00:00:00+09:00`. 현행 방침 11절이 "변경 사항의 시행 전 서비스 공지 또는 본 페이지를 통해 고지"를 약속하므로, 배포 후 시행일까지 공지 팝업(notices)으로 알린다. `findActive`는 시행일이 지난 버전만 보여 주므로 그 전까지 `/privacy`는 v4다.
- 태그 ID를 채우는 배포는 v5 시행일(10-05) 이후여야 한다. 고지가 수집보다 먼저여야 하고, 반대는 안 된다.
...
3. 배포 후 `yarn db:seed:terms`로 v5 적재. 공지 팝업(notices)으로 10-05 시행을 미리 알린다.
   - ID·라벨을 채운 코드는 10-05 이후에 배포한다.
