...
이벤트 fingerprint를 캐시 키에 접는다. 선택된 이벤트의 `publishedAt` 목록을 해시한다.

### 2단 좁히기 — 이력과 같은 제약

캐시 키는 캐시를 **조회하기 전에** 만들어져야 하고, 봉은 캐시 **미스 이후에** 가져온다.
그래서 봉 기준 절단을 캐시 키 계산에 쓸 수 없다. 이력(`priorAnalyses`)이 겪은 것과
똑같은 순서 제약이다.

- **stage 1 (캐시 키)**: 봉 없이 `impact` 필터만 적용해 fingerprint를 만든다.
  소비자가 넘긴 범위가 곧 후보다.
- **stage 2 (프롬프트)**: 봉이 확보된 뒤 `selectMarketEvents`가 봉 범위로 자르고
  상한을 적용한다.

두 단계의 결과가 다를 수 있다 — stage 1이 더 넓다. 그래도 안전한 이유는 소비자가
넘기는 범위(`MARKET_EVENT_LOOKBACK_DAYS`)를 봉 범위보다 넉넉히 잡되 **스트림과
pre-warm이 같은 값을 쓰기** 때문이다. 같은 봉을 보는 두 요청은 같은 stage 1 후보를
갖고, 따라서 같은 캐시 키를 만든다.
