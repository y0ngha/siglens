...
→ `privacy v3` 발행. 발효일 최초 설계는 **2026-09-19 00:00 KST**(v2 발효 9/9 +
10일)였으나 **2026-09-10 00:00 KST로 앞당겼다** — §14.6.

### 14.4 발효 전 배포를 코드가 스스로 막았다 (철회됨 — §14.6)

> **이 절의 설계는 더 이상 코드에 없다.** 아래는 왜 그런 장치가 있었는지에 대한
> 기록이다. `DIAGNOSTIC_COLUMNS_EFFECTIVE_AT`을 다시 넣지 말 것.

§7.4는 발효 시점을 "코드가 아니라 배포 순서로 푼다"고 했다. 이번엔 반대로 갔다 —
`presence` 라우트가 `DIAGNOSTIC_COLUMNS_EFFECTIVE_AT`(v3 발효 시각)을 직접 보고,
그 전에는 세 컬럼을 `null`로 남겼다.
...
4. 애플리케이션 배포 — 배포 즉시 세 컬럼 수집이 시작된다(§14.6)
5. 배포 후 `yarn metrics`로 UA 표 확인 → 크롤러가 보이면
   `src/shared/api/isBot.ts`에 토큰 추가

### 14.6 발효일 앞당김과 게이트 제거 (2026-09-10)

운영자 판단으로 수집을 즉시 시작하기로 했다. 두 가지를 함께 바꿔야 정합이 맞는다 —
게이트만 빼면 발효 전 방침(v2, "가명 식별자와 접속 일자"만)으로 미고지 수집이 되고,
발효일만 당기면 코드가 여전히 `null`을 쓴다.

1. `presence` 라우트에서 `DIAGNOSTIC_COLUMNS_EFFECTIVE_AT`과 `disclosed` 삼항을
   제거 — 세 컬럼을 항상 쓴다.
2. v3 발효일을 `2026-09-19` → `2026-09-10 00:00 KST`로. **`v3.md` 편집만으로는
   부족하다** — §8.2대로 `upsertFromSeed`가 `onConflictDoNothing`이라 이미 적재된
   행의 `effective_date`를 갱신하지 않는다. 운영 DB `terms` 행을 직접 `UPDATE`했고,
   시드 파일은 새 환경용으로 맞춰 둔 것이다.

순서는 **방침 먼저, 배포 나중**이다. 그래야 "방침은 넓은데 아직 안 걷는" 안전한
쪽으로만 어긋난다. 방침 페이지는 `revalidate = 86400`이라 DB만 바꾼 직후에는 최대
하루 v2가 나갈 수 있는데, 배포가 재빌드하면서 넘어간다.

보존 기간 집행(`RETENTION_DAYS` / `lastPrunedDate`)은 건드리지 않았다. 그건 수집
시작 조건이 아니라 방침이 고지한 400일을 집행하는 장치다.
