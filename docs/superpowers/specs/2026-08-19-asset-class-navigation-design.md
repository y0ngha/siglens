...
### 8.1 감사에서 드러난 것 — 처리 결과

이 절은 "안 보였다"와 "보고 미뤘다"를 구분하기 위한 기록이다.

**실측(2026-08-19, DB 미기록 프로브)**: core 0.48.0 배선 후 두 경제권 출력 확인.
경제 발표 8건(KR 5 / US 3) 전부 국가가 정확히 서술됐고, 옛 규칙이 뒤집던
**KR CPI 2.8% vs 예상 3.0% → `bullish`**가 바르게 나왔다. BSI 해설에 한국은행
오귀속 없음. 브리핑은 KR에서 `vixLevel` 필드 자체가 사라졌고(지어낸 숫자 소멸)
`riskSentiment`가 지수 격차·섹터 분산을 근거로 썼다. US는 `vixLevel: 15.84`로
입력 시세와 일치했다. 섹터명은 양쪽 모두 해당 시장의 실제 한국어명만 나왔다.
...
`BriefingCard`의 필터는 **유지한다.** 프롬프트를 고쳐도 출력은 모델 산물이라
규칙 준수가 보증되지 않는다(옛 캐시는 무관하다 —
`MARKET_BRIEFING_PROMPT_TEMPLATE_VERSION`이 키에 붙어 옛 엔트리는 도달 불가).
비교 대상만 심볼에서 한국어명으로 바꿨다.
