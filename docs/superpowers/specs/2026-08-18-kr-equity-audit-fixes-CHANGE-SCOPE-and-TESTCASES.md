...
## 3.6 로컬 실증 결과 (2026-08-18, 프로덕션 빌드 + curl)

`yarn build` → `yarn start`(포트 3117)로 실측. 브라우저 실증은 이번 범위 제외.

| # | 항목 | 결과 |
|---|---|---|
| 1 | `/005930.KS` 한글명 | ✅ `삼성전자` 노출 |
| 2 | 원화 표기 | ✅ `₩274,500`, USD 0건 |
| 3 | 한글 검색 인덱스 | ✅ `삼성전자`→`005930.KS`, `네이버`→`035420.KS` (검색은 서버 액션이라 DB 계층으로 확인) |
| 4 | KR 뉴스 탭 | ✅ `035720.KS`에서 ItemList JSON-LD 10건(headline·url·datePublished 정상) |
| 5 | 소문자 정규화 | ✅ `/005930.ks` → 301 → `/005930.KS` |
| 6 | sitemap lastmod | ✅ `2026-08-18T06:30Z` = KRX 마감 15:30 KST. 휴장일 광고 없음 |
| 7 | sitemap 스코프 | ✅ KR 120 URL = 20종목 × 6탭, `/options`·`/congress` 없음 |
| 8 | robots | ✅ KR sitemap과 무모순 |
| 9 | KR `/overall` 색인성 | ✅ `noindex, nofollow` |
| 10 | 미국 `/overall` 무영향 | ✅ `index, follow` |
| 11 | kr-tickers 크론 | ✅ 헤더 없이 401 (Bearer 게이트 정상) |

**실증 중 드러난 사실 2가지**

1. **KR 상위 3종목(005930·000660·005380)의 뉴스가 0건이다.** 4번째부터는 85~170건이다.
   이 PR에 포함된 `fe03ed30`(prewarm 회전이 KR 상위 종목을 영구 기아)이 아직 배포되지
   않아 생긴 프로덕션 데이터 공백이며, 브랜치 결함이 아니다. 배포 후 첫 prewarm
   사이클에서 채워지는지 확인해야 한다.
2. **마이그레이션 0028은 이미 프로덕션에 적용돼 있다** — `korean_tickers` 33,000행,
   `delisted_at` 컬럼 존재, 상폐 0건.

**실증 절차상 함정 2가지(다음에 반복하지 말 것)**

- 포트 3111에 **다른 워크트리(`siglens-integ`)의 낡은 서버**가 09:00부터 떠 있었다.
  그대로 curl했으면 옛 코드를 실증하고 통과로 읽었다. 서버의 `cwd`를 확인하고 시작할 것.
- `next start`는 `output: standalone`과 맞지 않는다는 경고를 낸다. 대신
  `.next/standalone/server.js`로 띄우면 그 디렉터리에서 `.env.local`을 못 읽어
  `DATABASE_URL` 부재로 전 페이지가 degrade한다. 둘 중 무엇을 쓰든 서버 로그에
  `degrading to []`가 있는지 먼저 볼 것.
