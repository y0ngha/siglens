...
### 4.5 Self-norm 경고 표시 룰

§9의 *만성 약세/강세 paradox*를 사용자에게 직접 노출하는 UI 장치. 점수 자체는 알고리즘 정의대로 산출하고 변경하지 않으며, *표시만 추가*한다.

다음 두 조건 중 하나를 만족할 때, 점수 옆에 작은 ⚠️ 아이콘과 hover/tap tooltip을 표시한다. **분석 탭 카드와 공포지수 탭에만** 노출하고 헤더 chip에는 노출하지 않는다 (시각적 노이즈 회피).

**조건 A — 만성 약세 paradox**
- `MA200 거리 < 0`  **AND**  `52w 위치 < 0.30`  **AND**  `최종 점수 ≥ 55`
- 표시 문구: "이 종목은 장기 약세 사이클입니다. 점수는 자기 분포 대비 상대적 위치를 의미합니다."

**조건 B — 만성 강세 paradox (대칭)**
- `MA200 거리 > 0`  **AND**  `52w 위치 > 0.70`  **AND**  `최종 점수 ≤ 45`
- 표시 문구: "이 종목은 장기 강세 사이클입니다. 점수는 자기 분포 대비 상대적 위치를 의미합니다."
...
export interface FearGreedSnapshot {
  // ... 기존 필드
  warning: FearGreedWarning;  // null이면 표시 없음
}
```

`computeFearGreedIndex`가 위 조건을 평가해 `warning` 필드에 채워 반환한다. UI는 이 필드만 보고 분기 표시한다.
