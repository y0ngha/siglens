...
## [범례 수정 실증 + 서브패널 라벨 겹침 | redesign-p1 | 2026-08-26]
- 실증(모바일 500px, MA 5개 + RSI·MACD·DMI·BB·Keltner·Donchian): 범례 상자 **266×492 → 333×70**, 차트 대비 200% → 28%. 가격 패널 **86 → 108px**. 배경판 `rgb(9,9,11)` 불투명, `maxHeight` 91.5px, `+N` 칩 존재. 서브에이전트가 "브라우저 없이 확인 못 했다"고 명시한 항목 중 넷이 여기서 확인됐다 — 다만 "50%를 보장한다"던 몫은 실측 **44%**였다(축·간격이 총합에서 빠진다). 방향은 맞고 크기가 달랐다.
- Violation(내 측정 실수): 체크박스로 밴드를 끈 **직후 페인트를 강제하지 않고** 캔버스를 재고 "밴드 차폐 가설이 반증됐다"고 단언했다. 캔버스가 옛 상태였다. 이 세션에서 내가 직접 기록해 둔 함정("canvas는 페인트를 강제하기 전까지 stale")을 그대로 어겼고, 그 무효한 측정으로 서브에이전트의 **옳은** 분석을 뒤집을 뻔했다.
  - Rule: 상태를 바꾼 뒤 캔버스를 재려면 **반드시 페인트를 강제**하고, 바꾸기 전/후 값이 실제로 달라졌는지부터 확인한다
  - Rule: 내가 만든 측정이 남의 분석을 뒤집을 때는 **측정 쪽을 먼저 의심**한다. 이 세션에서 그 방향으로 세 번 틀렸다(oklab 정규식, 없는 경로 grep, 이번 건)
- Context(진짜 답): 색 분포를 보니 `61,72,65`·`60,71,64` 계열이 3,400px, `131,110,44`·`70,60,28`이 700px — MA60 초록과 MA20 노랑이 어두운 배경 위에 **안티에일리어싱된 혼합색**이다. 선은 그려진다. 정확한 RGB에 도달하지 못할 뿐이고, 스크린샷으로 육안 확인했다. 서브에이전트의 래스터화 설명이 맞았다.
  - Rule: 캔버스에서 "0px"은 **안 그려졌다**가 아니라 **그 정확한 색이 없다**는 뜻이다. 얇은 선은 혼합색으로만 존재한다 — 색 분포를 보거나 눈으로 봐야 한다
- Violation(범례를 고치다 드러난 것): 스크린샷에서 **서브패널 라벨끼리 겹치는 것**이 보였다. `MACD(12,26,9)`/`Signal`/`Histogram` 아래로 `+DI(14)`/`ADX(14)`가 파고들었다. 범례는 pane 높이로 묶었는데 `usePaneLabels`가 만드는 라벨은 여전히 무제한이었다 — 부라벨 3개면 11px×3 + 간격 6px×2 = 45px로 36px 패널을 넘는다.
  - Rule: 같은 화면에 **겹칠 수 있는 요소가 둘 이상**이면 하나만 묶어도 문제가 남는다. 경계를 줄 때는 그 영역을 쓰는 것들을 함께 센다
  - Context: 가로 배치(`row` + `wrap`) + pane 높이 클램프(`maxHeight`, `overflow:hidden`)로 고쳤다. 가로면 한 줄 11px면 끝난다. pane 재분배 시 클램프도 함께 갱신한다. 방향과 클램프를 **함께** 단언하는 테스트를 넣고 각각 되돌려 red를 확인했다 — 하나만 보면 다른 하나가 사라져도 통과한다.

  **정정(R16 감사)**: 위 "각각 되돌려 red를 확인했다"는 **생성 시점 클램프에만** 해당한다. 실제로 되돌려 본 것은 `flexDirection`과 `overflow` 두 가지였고, `recomputeTops`·RAF·`ResizeObserver`를 포함한 **반응 경로 전체를 지워도 전체 스위트가 초록**이었다. 즉 "pane 재분배 시 갱신한다"는 동작은 그때 아무 게이트도 지키지 않았다. 검증한 범위보다 넓게 적은 것이고, 이 로그에서 같은 유형을 두 번째로 저질렀다(R8의 "공유했다" 건).
  - Rule: 무엇을 되돌려 봤는지 **그 목록 그대로** 적는다. "각각"이라는 말은 열거한 것에만 걸린다

## [배포 전 5종 감사 R16 | redesign-p1 | 2026-08-26]
- 결과: SEO 0건(2라운드 연속), 빌드 1건(내 주석 오류, 수정), 렌더 1건(판단 요청 — 이미 구현돼 있어 조치 없음), 코드 6건(블로커 1), 지표 4건. 접근성 수정 5건과 범례 수정 4건이 **브라우저 수치로 전부 검증**됐다: 인증 링 1.78/1.96 → 5.16~6.70, `.focus-glow` 실선 링 5.41~6.31 추가, 달력 포커스가 이제 실제로 달라짐, 백테스팅 h2 17개, skip link가 `activeElement`를 옮김. 범례는 107px 패널 안 333×70, 대비 4.5 미만 행 **0개**(이전 11/33).
- Violation(블로커): **기능 전체가 호출부에서 조용히 뽑힐 수 있었다.** `usePricePaneStretch`·`usePricePaneSize`·범례 두 prop을 `StockChart`에서 전부 지워도 typecheck·lint·전체 스위트가 기준선과 **바이트 동일**했다. 훅 단위 테스트는 훅을 직접 렌더해서 통과하고, 호출부가 그걸 부르는지는 아무도 단언하지 않았다.
  - Rule: 훅을 테스트했다고 **배선을 테스트한 것이 아니다.** 새 훅을 붙일 때는 호출부에서 그 효과가 실제로 나타나는지 한 줄이라도 단언한다
  - Rule: 이 루프에서 여덟 번째다. "고쳐놓고 아무도 안 지키는 수정"은 이 프로젝트의 **지배적 결함 유형**이다
- Violation: **같은 문제를 가진 둘 중 하나만 고쳤다 — 두 번째로.** 직전 라운드에 범례 높이를 묶으며 "같은 영역을 쓰는 요소가 둘인데 하나만 묶었다"고 스스로 기록해 놓고, 이번엔 **대비에서 똑같이** 반복했다. 가격 범례에 배경판을 주고 서브패널 라벨은 그대로 뒀다(다크 최악 1.13:1). 게다가 내 서브패널 축소가 "선이 라벨을 가로지르는" 조건을 일상으로 만들었다. 볼륨 패널(선이 라벨에 안 닿음)만 12.08:1로 깨끗한 것이 원인을 정확히 가리켰다.
  - Rule: 한 결함을 고칠 때 **같은 성질을 가진 형제를 먼저 열거**한다. "이건 여기만"이라는 직관이 두 라운드 연속 틀렸다
- Violation: 내 수정이 **다른 결함의 발생 조건을 흔하게** 만들었다. TradingView 로고가 마지막 서브패널 라벨을 덮는 건 패널 43px 이하에서 나는데, 서브패널 몫을 `1/(2+N)`에서 `50%/N`으로 줄이면서 N=3부터 그 조건에 들어가게 됐다. N=6에서는 `● MFI`가 통째로 가려진다.
  - Rule: 레이아웃 비율을 바꾸면 **그 비율에 의존하던 다른 요소**를 다시 센다
- Context(인과 검증이 두 번 이겼다): 감사가 지목한 원인이 이번 라운드에만 두 번 틀렸다 — MA 0px의 "z-order 설"(실제로는 pane stretch 분배 + 래스터화), 그리고 내가 무효한 측정으로 세운 "밴드 반증". 둘 다 라이브러리 소스와 실측으로 다시 세웠다. 수정 에이전트도 내 브리핑을 그대로 믿지 않고 `lightweight-charts@5.2.0` dev 번들에서 `_adjustSizeImpl`과 `_pressedMouseMoveEvent`를 직접 확인했다.
- 수정 8건: 호출부 배선 단언 2개, `usePaneLabels` 반응 경로 테스트 4개(+ 중복된 top/maxHeight 계산을 `positionLabel()` 하나로 접어 한쪽만 고치는 삭제가 불가능하게), stretch를 개수가 아니라 **실제 stretch 합**으로 계산해 드래그 후에도 불변식이 참이 되게, 서브패널 픽셀 하한(`MIN_SUB_PANE_PX=30`)과 전체 높이에서 유도한 상한, 범례 폭 상수의 **방향** 단언, `usePricePaneSize` cleanup 단언, 서브패널 라벨 배경판(테마 게터로 읽어 리마운트를 타게), 로고 실측 rect 기반 라벨 회피. 여덟 건 모두 되돌려 red를 확인했다.

## [배포 전 5종 감사 R17 — 코드·테스트 | redesign-p1 | 2026-08-26]
- 결과: 3건 전부 경미, 전부 **뮤테이션 생존자**(붙들어 두는 것이 없는 불변식). 실동작 결함 0. 감사가 앞선 8건 수정을 독립 재검증했고(13회 되돌림, 전부 red) 이번엔 fix-log 주장이 실제로 성립했다. 가드 11종도 **제품 소스에 진짜 위반을 심어** 전부 red를 확인했다 — 자기 테스트에 합성 문자열을 넣는 방식이면 파일 순회가 아무것도 못 읽어도 통과하기 때문이다.
- Violation: **한 파일에서 고친 유형을 형제 파일에 안 적용했다 — 같은 커밋 안에서 두 번.** (1) `overlayLegendLayout`의 폭 상수를 "값이 아니라 방향으로" 단언하게 고쳐놓고, 형제인 `MIN_PRICE_PANE_STRETCH`는 부등호 양쪽에 같은 상수를 써서 2→1로 낮춰도 전부 초록이었다. (2) `usePricePaneSize`에 cleanup 단언을 추가해놓고, 같은 RAF 패턴을 쓰는 형제 훅 둘 — 그중 하나는 **그 커밋이 새로 만든 코드** — 에는 안 넣어 `cancelAnimationFrame`을 지워도 초록이었다.
  - Rule: 이 루프에서 **"둘 중 하나만 고침"이 세 번째**다(범례↔라벨 기하, 범례↔라벨 대비, 이번 두 건). 결함 유형을 하나 고쳤으면 **그 유형을 가진 형제를 그 자리에서 grep**한다. "이건 여기만"이라는 직관이 세 라운드 연속 틀렸다
  - Context: 하한을 낮췄을 때의 결과까지 계산됐다 — 214px 모바일·보조 6개에서 픽셀 상한이 하한을 이겨 가격 pane이 53.5px(기본 배치)에서 34px로 내려앉는다. 모듈 헤더가 "그래도 안 되면 LWC 기본 배치로 돌아간다"고 약속한 것과 **반대 방향**이다. 그래서 리터럴 대신 **기본 배치와 견주는** 단언으로 고쳤다(보조 1~8개 × 예산 4종).
- Context: 로고 회피의 "마크 없음" 분기가 무단언이었다 — 다른 테스트가 전부 마크를 심어 두고 돌아, `999px`를 반환해도 통과했다. 마크를 안 붙인 컨테이너로 도는 테스트를 넣었다. 세 건 모두 되돌려 red 확인.

## [배포 전 5종 감사 R17 — 브라우저 기하 | redesign-p1 | 2026-08-26]
- 결과: 저자가 "브라우저 없이 검증 불가"로 남긴 6가지가 **전부 실측됐다**. 패널 분할은 N=1~8에서 공식과 반올림 오차 내 일치(N=4에서 보조 pane 정확히 30.0px), 라벨 배경판 라이트 **12.46:1** · 다크 **14.67:1**(이전 1.13 / 2.93), 로고 회피는 전 구성에서 교차 0(라벨 2개 동시 밀림 포함), 한 프레임 튐 없음.
- Context(가장 값진 검증): **드래그 케이스를 실제 신뢰 입력으로 증명했다.** N=2에서 구분선을 끌어 보조 stretch를 0.5556/2.4444/1로 만든 뒤 DMI를 켜니 가격 pane이 정확히 50.0% — 개수 기반이던 옛 코드면 40.3%였을 값이다. 합산 방식 수정이 살아 있다는 것을 유일하게 증명할 수 있는 경로였다.
- Context(감사가 자기 함정 둘을 스스로 잡음): `computer` 좌표가 CSS px가 아니라 **스크린샷 공간**(×1.722)이라 토글 클릭이 엉뚱한 곳에 떨어져 "토글 고장"으로 보고할 뻔했다. 그리고 숨김 탭에서는 rAF가 안 돌아 **레이아웃 자체가 stale**이라(LWC의 `_adjustSizeImpl`이 rAF 예약) 드래그를 "무반응"으로 볼 뻔했다.
  - Rule: 숨김 탭에서 stale한 것은 캔버스 픽셀만이 아니다. **레이아웃도 stale하다** — 기하를 재기 전에도 페인트를 강제한다
- Violation: 짧은 pane에서 두 줄 라벨이 **둘째 줄 중간을 가로질러** 잘렸다. 글자 윗부분 2~3px만 남은 얼룩이 된다. 줄 단위로 내림해 온전한 줄까지만 남기도록 고쳤다.
- Violation: `positionLabel`이 높이를 먼저 재고 나중에 `left`를 옮겼다 — `left`가 바뀌면 남는 폭이 바뀌고 그러면 줄바꿈과 높이가 바뀌므로, **판단의 근거가 그 판단 때문에 무효가 되는** 순서였다. 실측 케이스에서 줄 수가 안 바뀌어 틀린 적은 없었지만 잠복이라 순서를 바로잡았다.
  - Violation(그 위에 또): 순서를 고치고 **뮤테이션을 돌려보니 초록이었다.** jsdom은 높이가 0이라 두 순서가 구분되지 않는다. 라벨이 두 줄 높이를 보고하게 만들고 마크 띠를 "한 줄이면 안 겹치고 두 줄이면 겹치는" 위치에 두어 구분되게 했다. **고친 직후 지워보는 습관이 또 한 번 값을 했다**
- Violation: `MIN_SUB_PANE_PX` 주석이 "보조 pane **하나가** 지켜야 할 최소 높이"라고 했는데, 상한은 **합**에만 걸린다. 드래그로 stretch가 불균등해지면 합은 지켜지면서 개별 pane이 22px까지 내려간다(실측 54/22/22/22, 합 120 = 4×30). 동작이 아니라 문서가 틀렸으므로 문서를 실제 보장에 맞췄다.
  - Rule: 상수 주석은 **그 상수가 실제로 강제하는 것**을 적는다. 의도를 적으면 다음 사람이 없는 보장을 믿는다

## [PR #771 Claude 리뷰 R1 | redesign-p1 | 2026-08-26]
- 결과: 인라인 블로커 11건. **하나도 false positive가 아니었다** — 11건이 인용한 규칙 원문(CONVENTIONS.md ESLint Rules, MISTAKES.md #13·#18·#22)이 전부 파일에 실재했고, 지목한 코드도 전부 그 자리에 있었다. 리뷰 봇 주장은 검증 후 적용이 원칙이지만 이번엔 검증이 전부 통과 쪽으로 끝났다.
- Violation: **억제 주석 두 개가 억제하는 대상이 없었다.** `layout.tsx`·`global-error.tsx`의 `// eslint-disable-next-line react/no-danger`는 `.oxlintrc.json`에 그 규칙이 아예 없어 한 번도 발동한 적이 없다. 즉 lint를 조용히 만든 게 아니라 **규약 위반처럼 보이는 표식만 남긴** 것이다. 리뷰어는 그 표식을 읽는다.
  - Rule: 억제 주석을 쓰기 전에 그 규칙이 **이 프로젝트에서 켜져 있는지** 확인한다. 꺼진 규칙에 붙인 억제는 비용만 있고 효과가 없다
  - Context: 같은 스크립트 주입이 두 셸에 복사돼 있던 것도 함께 드러났다. `shared/ui/ThemeInitScript.tsx` 하나로 접어 주입점을 1곳으로 줄였다.
- Violation: `VolumeChart`의 `exhaustive-deps` 억제는 **MISTAKES.md #13이 명시한 대안으로 그대로 풀렸다.** 게터(`CHART_COLORS.bullish`)를 콜백 안에서 부르니 린터에게 의존이 안 보였을 뿐이고, 읽은 값을 지역 const로 꺼내 deps에 넣자 억제가 필요 없어졌다. `useThemeVersion()`은 리렌더 유발이 유일한 일이므로 반환값을 묶지 않는다.
  - Rule: "린터가 못 보는 진짜 의존"은 대개 **값을 꺼내면 보이는 의존**이다. 억제하기 전에 한 번 꺼내 본다
- Violation: **`disabled:opacity-*`가 트리에 12곳 더 살아 있었다.** MISTAKES.md #18은 이 루프에서 이미 두 번(W6b·W6c) 기록된 규칙인데, 인스턴스만 고치고 형제를 세지 않아 9개 파일 12곳이 남았다. 양 테마 실측 결과 22조합 중 20조합이 4.5:1 미달이었고 **라이트는 전 사이트 실패**(최저 1.51:1)였다.
  - Rule: 이 루프에서 **"둘 중 하나만 고침"이 네 번째**다(범례↔라벨 기하, 범례↔라벨 대비, 형제 훅 둘, 이번 12곳). 세 번째에 "그 자리에서 grep한다"고 적어 놓고 네 번째를 냈다 — 규칙을 적는 것과 지키는 것이 다르다는 증거다. **정적 가드가 없으면 규칙은 다시 드리프트한다**
  - 조치: `disabledOpacityGuard`를 세워 계열 전체를 막았다. MISTAKES #18에 실측 대안 3종과 가드 경로를 적었다.
- Violation: 홈의 h2 둘이 공유 토큰 `HEADING_SECTION`(18px)을 두고 각자 `text-base`(16px) 리터럴을 복제하고 있었다. 두 파일의 주석이 **서로를 가리키며 "같은 톤으로 맞춘다"**고 적혀 있었는데, 정작 둘 다 토큰을 안 쓰고 토큰과 다른 값으로 함께 어긋나 있었다. 주석이 일치를 약속하는 동안 코드는 토큰에서 멀어졌다.
  - Rule: "다른 파일과 톤을 맞춘다"는 주석이 보이면 **맞출 대상이 토큰인지 리터럴인지** 본다. 리터럴끼리 맞춘 일치는 토큰과의 불일치다
