...
시트는 `h-[97svh]` 고정인데 vaul은 오프셋을 `(1 − snap) × window.innerHeight`로 잡는다.
두 단위가 일치할 때 **보이는 띠 = snap − 0.03**으로 정리되며, 실측이 이 식을 두 번 확인해 줬다
(snap 0.15 → 0.12, snap 0.20 → 0.17).

`SNAP_PEEK`는 최종적으로 **0.20**을 쓴다. 처음 잡았던 0.15는 띠가 0.12에 불과해,
`innerHeight`가 `svh`보다 `0.141·svh`(iPhone SE 기준 ~80px)만 커져도 띠가 0으로 수렴한다 —
모바일 툴바 접힘의 통상 범위(60~110px) 안이다. **시트를 여는 버튼이 따로 없어**
띠가 사라지면 AI 분석 패널에 접근할 방법이 없어지므로, 여유를 0.141·svh → 0.21·svh로
넓히는 쪽을 택했다.

| 기기 | 수정 전 시트 점유 | 수정 후 (snap 0.20) | 캔들 차트 | 거래량 차트 |
|---|---|---|---|---|
| iPhone SE | 0.52 | **0.171** | bottom 397 < sheetTop 471 ✅ | 428 < 471 ✅ |
| iPhone 14 | 0.52 | **0.170** | 455 < 551 ✅ | 494 < 551 ✅ |
| Pixel 7 | 0.52 | **0.170** | 596 < 696 ✅ | 681 < 696 ✅ |

**트레이드오프를 명시해 둔다**: 차트 하단과 시트 상단 사이 여유가 snap 0.15일 때의 62px에서
Pixel 7 기준 **15px**로 줄었다(가장 빡빡한 기기). 커버리지 계약 자체는 3기기 모두 만족한다.
이 방향을 택한 이유는 심각도가 비대칭이기 때문이다 — 띠 소실은 **기능 상실**(재로드 전까지
분석 패널 접근 불가)인 반면, 차트가 몇 px 가려지는 것은 사용자가 시트를 내려 즉시 복구할 수
있는 미관 문제다.

"AI 분석 중…" 배너는 띠 안에 그대로 유지된다(육안 확인).

**후속 과제 반영 완료**: 이 PR 내 후속 커밋에서 시트를 여는 명시적 버튼("AI 분석 보기",
`SymbolPageClient.tsx`의 timeframe bar에 위치, `onClick={() => setSheetSnap(SNAP_FULL)}`)을
추가했다. 띠가 0에 수렴하는 파국적 케이스는 이제 이 버튼으로 커버되며, 위 여유는 여전히
"AI 분석 중" 배너 가시성과 드래그-확장 조작성을 위해 유효하다.
...
## 배포 전 5종 감사에서 추가로 잡힌 것

| 지적 | 조치 |
|---|---|
| 모바일 팝오버가 짧은 뷰포트(iPhone SE 가로 375×667)에서 잘리고 **스크롤 경로가 없음** — `items-center` + 스크롤 없는 fixed 컨테이너 | 백드롭을 `items-start overflow-y-auto`로, 패널에 `my-auto` — 짧을 땐 중앙 정렬, 길면 스크롤 |
| 모바일 팝오버가 실질적 모달인데 `aria-modal` 없음 | 모바일 경로에만 부여(데스크탑 앵커드 팝오버는 종전대로 미부여) |
| 뷰포트가 뒤집히면(기기 회전) 패널 노드가 교체되며 포커스 트랩이 조용히 죽음 | `useFocusTrap`에 선택적 `rearmKey` 추가. rearmKey를 deps에서 빼면 실패함을 red/green으로 확인 |
| `formatSnapshotAsOf`가 무효 Date에서 렌더 중 `RangeError` (3개 감사가 독립 지적) | `string \| null` 반환 + 셸이 고정 캡션으로 폴백 + 저장소 경계에서 무효 행 제거 |
| 실패할 수 없는 테스트 4건 | 전부 교체 후, 각각 옛 동작을 되살려 red 확인 |
