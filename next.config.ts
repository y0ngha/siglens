...
const nextConfig: NextConfig = {
     ...
     *   - ETag를 살리면서 오염만 막는 길도 있다: CloudFlare가 강한 ETag를 **존중하지 않으면**
     *     압축하면서 약한 ETag(`W/`)로 바꿔 준다(CloudFlare ETag 문서 기준, 이 존에서는 미실측 —
     *     R1 룰 "Respect strong ETags" OFF,
     *     `docs/architecture/CDN_CACHING.md`). 대시보드 설정에 기대는 방식이라 코드에서 끊는
     *     쪽을 택했지만, 재방문 304가 아쉬우면 그 토글을 확인·OFF한 뒤 이 플래그를 되돌려도 된다.
