...
# 10–12시로 옮겼다. 창 총량은 126 tick(10.5h) → 109 tick(9h5m)으로 1시간 25분
# 줄었고(:05 시작 두 창의 tick 손실 포함), peak 노출은 0이 된다. **이 시각들을
# 옮길 때는 반드시 위 peak 구간을 피할 것** — 한 시간만 밀려도 그 시간대
# 토큰이 조용히 2배로 청구되고, 어떤 알람도 울리지 않는다.
#
# 창을 좁힌 대가(처리량 −1h25m)는 같은 감사에서 함께 잡은 두 건 — overall 축의
# technical 중복 생성 제거(core 1.13.1 + siglens 배선 — 배선은 2026-09-24에야
# 들어갔다), 느린 3탭(fundamental·financials·congress)의
...
# 밀렸다. `siglens-seo-prewarm-kr-boundary`(10:05–12:55 UTC = 19:05–21:55 KST;
